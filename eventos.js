...
const STORE='eventos_premium_v57';
const META='eventos_premium_meta_v57';
...
let state={tab:'dashboard',eventos:[],pacotes:window.EVENTOS_PACOTES||[],meta:{metaMensal:150000},cal:{year:new Date().getFullYear(),month:new Date().getMonth()+1}};
...
function renderWeekEvents(){
  ...
  if(!list.length){
    list=state.eventos.filter(e=>e.status==='Fechado'||e.status==='Realizado').sort((a,b)=>String(b.data).localeCompare(String(a.data))).slice(0,10);
  }
...
$('dashboard').innerHTML=`<div class="dash-premium">
  <div class="dash-main-card">
    <span class="eyebrow">Visão comercial</span>
    <h2>${brl(c.total)}</h2>
    <p>Total estimado no filtro atual</p>
    <div class="progress"><i style="width:${pct}%"></i></div>
    <small>Meta mensal: ${brl(meta)} · ${pct.toFixed(0)}%</small>
  </div>
  <div class="dash-mini-grid">
    <div class="kpi compact"><div class="label">Eventos</div><div class="value">${c.count}</div><div class="hint">${c.fechados} fechados/realizados</div></div>
    <div class="kpi compact"><div class="label">Almoço</div><div class="value">${brl(c.almoco)}</div><div class="hint">Turno almoço</div></div>
    <div class="kpi compact"><div class="label">Jantar</div><div class="value">${brl(c.jantar)}</div><div class="hint">Turno jantar</div></div>
    <div class="kpi compact"><div class="label">Ticket médio</div><div class="value">${brl(c.ticket)}</div><div class="hint">Por evento</div></div>
    <div class="kpi compact"><div class="label">Conversão</div><div class="value">${c.conv.toFixed(1)}%</div><div class="hint">Fechados / total</div></div>
    <div class="kpi compact"><div class="label">Gorjeta</div><div class="value">${brl(c.gorjeta)}</div><div class="hint">Campo por evento</div></div>
  </div>
</div>
<div class="grid-2 premium-grid">
  <div class="panel premium-panel"><h3>Vendas por pacote/proposta</h3>${bars(byPkg)}</div>
  <div class="panel premium-panel"><h3>Dias da semana</h3>${bars(byDow)}</div>
  <div class="panel premium-panel"><h3>Turnos</h3>${bars(byTurno)}</div>
  <div class="panel premium-panel"><h3>Status do funil</h3>${bars(byStatus)}</div>
</div>
<div class="panel meta-inline"><h3>Meta mensal</h3><input class="field" type="number" value="${meta}" onchange="EVENTOS.setMeta(this.value)"></div>`;
}
...
function renderCalendar(){
  ...
  const years=[2026,2027,new Date().getFullYear()].filter((v,i,a)=>a.indexOf(v)===i).sort();
