...
function effectiveAlarm(c){const local=getLocalAlarm(c.id); if(local&&local.enabled) return Object.assign({scope:'local'},local); if(c.sharedAlarmEnabled&&c.sharedAlarmTime) return {scope:'shared',enabled:true,time:c.sharedAlarmTime,advance:Number(c.sharedAlarmAdvance||0),vibrate:c.sharedAlarmVibrate!==false,sound:c.sharedAlarmSound!==false,soundMode:'default',soundName:'Som padrão'}; return null;}
function readAudioFileAsDataURL(file){return new Promise((resolve,reject)=>{ if(!file) return resolve(null); if(!file.type||!file.type.startsWith('audio/')) return reject(new Error('Arquivo inválido')); if(file.size>2.5*1024*1024) return reject(new Error('Arquivo muito pesado')); const r=new FileReader(); r.onload=()=>resolve({name:file.name,dataUrl:String(r.result||'')}); r.onerror=()=>reject(new Error('Falha ao ler arquivo')); r.readAsDataURL(file); });}
function playAlarmSound(a){ if(!a||a.sound===false) return; const custom=(a.scope==='local' && a.soundMode==='custom' && a.soundDataUrl); if(custom){ try{ const audio=new Audio(a.soundDataUrl); audio.volume=.9; const play=audio.play(); if(play&&play.catch) play.catch(()=>beep()); return; }catch(e){ beep(); return; } } beep(); }
...
function openCardForm(listId,preset={}){modalForm('Criar cartão',`<div class="gt-field"><label>Título</label><input id="fTitle" value="${escAttr(preset.title||'')}"></div><div class="gt-field"><label>Prazo</label><input id="fDue" type="date" value="${preset.due||''}"></div><div class="gt-field"><label>Atividade fixa/recorrente</label><select id="fRec"><option value="none">Não repetir</option><option value="daily">Diária</option><option value="weekdays">Segunda a sexta</option><option value="weekly">Semanal</option><option value="monthly">Mensal</option></select></div><div class="gt-field"><label>Despertador / notificação</label><div class="gt-reminder-row"><input id="fAlarmTime" type="time"><select id="fAlarmAdvance"><option value="0">Na hora</option><option value="5">5 min antes</option><option value="10">10 min antes</option><option value="15">15 min antes</option><option value="30">30 min antes</option><option value="60">1h antes</option></select></div><select id="fAlarmScope"><option value="local">Somente neste dispositivo</option><option value="shared">Compartilhado para todos</option></select><small>Ao informar horário, o celular pode notificar, vibrar e tocar som quando permitido. O toque personalizado pode ser definido depois ao abrir o card.</small></div>`,()=>{const title=$('#fTitle').value.trim(); if(!title)return toast('Informe o título'); const alarmTime=$('#fAlarmTime').value; const c={id:uid('c'),title,description:'',labels:[],due:$('#fDue').value,done:false,recurrence:$('#fRec').value,sharedAlarmEnabled:false,sharedAlarmTime:'',sharedAlarmAdvance:0,sharedAlarmVibrate:true,sharedAlarmSound:true,checklist:[],comments:[],createdAt:new Date().toISOString(),updatedAt:new Date().toISOString()}; state.cards[c.id]=c; if(alarmTime){const sc=$('#fAlarmScope')?.value||'local'; if(sc==='shared'){c.sharedAlarmEnabled=true; c.sharedAlarmTime=alarmTime; c.sharedAlarmAdvance=Number($('#fAlarmAdvance').value||0); c.sharedAlarmVibrate=true; c.sharedAlarmSound=true;} else {setLocalAlarm(c.id,{enabled:true,time:alarmTime,advance:Number($('#fAlarmAdvance').value||0),vibrate:true,sound:true,soundMode:'default',soundName:'Som padrão'});} requestNotifyPermission();} state.lists[listId].cards.push(c.id); save(); closeModals(); renderBoard();}); setTimeout(()=>{$('#fTitle')?.focus();},50)}
...
function chooseAlarm(c){
 const a=effectiveAlarm(c)||{scope:'local',enabled:false,time:'09:00',advance:0,vibrate:true,sound:true,soundMode:'default',soundName:'Som padrão'};
 const customInfo=(a.scope==='local'&&a.soundMode==='custom'&&a.soundName)?`<div class="gt-inbox-item" id="alarmSoundInfo"><strong>Toque atual:</strong><br><small>${esc(a.soundName)}</small></div>`:`<div class="gt-inbox-item" id="alarmSoundInfo"><strong>Toque atual:</strong><br><small>Som padrão do sistema</small></div>`;
 modalForm('Despertador / notificação',`<label class="gt-check-line"><input id="alarmEnabled" type="checkbox" ${a.enabled?'checked':''}> Ativar lembrete para este card</label>
 <div class="gt-field"><label>Onde este alarme deve tocar?</label><select id="alarmScope"><option value="local">Somente neste dispositivo</option><option value="shared">Compartilhado para todos</option></select><small><b>Somente neste dispositivo:</b> toca apenas no celular/computador que configurou. <b>Compartilhado:</b> sincroniza no Firebase e toca para todos com o app aberto e permissão ativa.</small></div>
 <div class="gt-field"><label>Data do prazo</label><input id="alarmDue" type="date" value="${c.due||todayISO()}"></div>
 <div class="gt-field"><label>Horário do alarme</label><input id="alarmTime" type="time" value="${a.time||'09:00'}"></div>
 <div class="gt-field"><label>Quando avisar</label><select id="alarmAdvance"><option value="0">Na hora</option><option value="5">5 minutos antes</option><option value="10">10 minutos antes</option><option value="15">15 minutos antes</option><option value="30">30 minutos antes</option><option value="60">1 hora antes</option></select></div>
 <label class="gt-check-line"><input id="alarmVibrate" type="checkbox" ${a.vibrate!==false?'checked':''}> Vibrar no celular quando possível</label>
 <label class="gt-check-line"><input id="alarmSound" type="checkbox" ${a.sound!==false?'checked':''}> Tocar som no navegador</label>
 <div class="gt-field" id="alarmSoundBox"><label>Toque do alarme neste dispositivo</label>${customInfo}<input id="alarmAudioFile" type="file" accept="audio/*"><small>Você pode selecionar um áudio do aparelho. Por segurança do navegador, esse toque fica salvo apenas neste dispositivo. Para alarme compartilhado, cada aparelho usa o próprio toque local ou o som padrão.</small><button type="button" class="gt-secondary" id="alarmClearSound">Usar som padrão</button></div>
 <p class="gt-muted">Para tocar/notificar, o navegador precisa permitir notificações. No iPhone, funciona melhor com o app instalado na tela inicial.</p>
 <button type="button" class="gt-secondary" id="alarmPermission">Permitir notificações</button><button type="button" class="gt-secondary" id="alarmTest">Testar alarme agora</button>`,async()=>{
   const enabled=$('#alarmEnabled').checked; const scope=$('#alarmScope').value; c.due=$('#alarmDue').value;
   let currentLocal=getLocalAlarm(c.id)||{};
   let cfg={enabled,time:$('#alarmTime').value,advance:Number($('#alarmAdvance').value||0),vibrate:$('#alarmVibrate').checked,sound:$('#alarmSound').checked,soundMode:currentLocal.soundMode||'default',soundName:currentLocal.soundName||'Som padrão',soundDataUrl:currentLocal.soundDataUrl||''};
   const file=$('#alarmAudioFile')?.files?.[0];
   if(file){ try{ const audio=await readAudioFileAsDataURL(file); cfg.soundMode='custom'; cfg.soundName=audio.name; cfg.soundDataUrl=audio.dataUrl; }catch(e){ toast(e.message==='Arquivo muito pesado'?'Áudio muito pesado. Use um arquivo de até 2,5 MB.':'Selecione um arquivo de áudio válido.'); return; } }
   if(scope==='local'){
     setLocalAlarm(c.id,cfg); c.sharedAlarmEnabled=false; c.sharedAlarmTime='';
   }else{
     // O alarme compartilhado vai para o Firebase, mas o arquivo de áudio escolhido no aparelho não é enviado para todos.
     // Mantemos o toque local salvo no dispositivo para que esta pessoa use o áudio escolhido quando o card tocar aqui.
     if(cfg.soundMode==='custom'&&cfg.soundDataUrl){ setLocalAlarm(c.id,Object.assign({},cfg,{enabled:true})); }
     else removeLocalAlarm(c.id);
     c.sharedAlarmEnabled=enabled; c.sharedAlarmTime=cfg.time; c.sharedAlarmAdvance=cfg.advance; c.sharedAlarmVibrate=cfg.vibrate; c.sharedAlarmSound=cfg.sound;
   }
   c.updatedAt=new Date().toISOString(); if(enabled) requestNotifyPermission(); save(); closeModals(); renderBoard();
 });
 setTimeout(()=>{
   $('#alarmScope').value=a.scope||'local'; $('#alarmAdvance').value=String(a.advance||0);
   const updateSoundBox=()=>{ const isShared=$('#alarmScope').value==='shared'; const box=$('#alarmSoundBox'); if(box){ box.style.opacity=isShared?.75:1; } };
   $('#alarmScope').onchange=updateSoundBox; updateSoundBox();
   $('#alarmPermission').onclick=requestNotifyPermission;
   $('#alarmTest').onclick=()=>fireCardAlarm(c, true);
   $('#alarmClearSound').onclick=()=>{const la=getLocalAlarm(c.id)||{}; delete la.soundDataUrl; la.soundMode='default'; la.soundName='Som padrão'; if(la.enabled) setLocalAlarm(c.id,la); const info=$('#alarmSoundInfo'); if(info) info.innerHTML='<strong>Toque atual:</strong><br><small>Som padrão do sistema</small>'; const inp=$('#alarmAudioFile'); if(inp) inp.value=''; toast('Toque padrão selecionado. Clique em Salvar para confirmar.');};
   $('#alarmAudioFile').onchange=()=>{const f=$('#alarmAudioFile').files?.[0]; const info=$('#alarmSoundInfo'); if(f&&info) info.innerHTML=`<strong>Novo toque selecionado:</strong><br><small>${esc(f.name)}</small>`;};
 },20)
}
...
function fireCardAlarm(c,test=false){ const a=effectiveAlarm(c)||{sound:true,vibrate:true,scope:'local',time:'',soundMode:'default'}; playAlarmSound(a); if(a.vibrate!==false && navigator.vibrate) navigator.vibrate([280,120,280,120,280]); const tipo=a.scope==='shared'?'Alarme compartilhado':'Alarme deste dispositivo'; const toque=a.soundMode==='custom'&&a.soundName?` • Toque: ${a.soundName}`:''; const body=`${tipo} • ${a.time||''}${toque}${test?' • Teste':''}`; if('Notification' in window && Notification.permission==='granted'){try{new Notification('Gerenciador de Tarefas', {body:`${c.title}\n${body}`, icon:'icon.png', badge:'icon.png'});}catch(e){toast('⏰ '+c.title)}} else toast('⏰ '+c.title); }
