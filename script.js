...
// ═══════════════════════════════════════════════════
// FREQUÊNCIA POR CICLO DE ESCALA — DIA 21 → DIA 20
// ═══════════════════════════════════════════════════
function _mesesFrequencia(){
  return ['JANEIRO','FEVEREIRO','MARCO','ABRIL','MAIO','JUNHO','JULHO','AGOSTO','SETEMBRO','OUTUBRO','NOVEMBRO','DEZEMBRO'];
}
function _normalizarTextoPeriodo(s){
...
function normalizarNomeAbaPeriodo(nome){
  let s=_normalizarTextoPeriodo(nome||'');
  s=s.replace(/^COPY OF\s+/,'').replace(/\s+/g,' ').trim();
  const m=s.match(/([A-Z]+)\s*\/\s*([A-Z]+)\s*(\d{2,4})?/);
  const meses=_mesesFrequencia();
  if(!m) return _abaAtual();
  const mi=meses.indexOf(m[1]);
  const mf=meses.indexOf(m[2]);
  if(mi<0||mf<0) return _abaAtual();
  let ano=m[3]||String(new Date().getFullYear()).slice(-2);
  ano=ano.length===4?ano.slice(-2):ano.padStart(2,'0');
  return `${meses[mi]} / ${meses[mf]} ${ano}`;
}
function getNomeAbaFrequenciaAtual(){
  const salvo=load('freq_aba_atual',null);
  if(salvo) return normalizarNomeAbaPeriodo(salvo);
  if(typeof ESCALA_ABA!=='undefined' && ESCALA_ABA) return normalizarNomeAbaPeriodo(ESCALA_ABA);
  return _abaAtual();
}
function setNomeAbaFrequenciaAtual(nome){
  const n=normalizarNomeAbaPeriodo(nome);
  save('freq_aba_atual', n);
  localStorage.setItem('freq_aba_atual', n);
  return n;
}
function parsePeriodoFrequencia(nome){
  const aba=normalizarNomeAbaPeriodo(nome);
  const meses=_mesesFrequencia();
  const m=aba.match(/([A-Z]+)\s*\/\s*([A-Z]+)\s*(\d{2})/);
  const hoje=new Date();
  if(!m){
    const mes=hoje.getMonth();
    const ano=hoje.getFullYear();
    return {aba:_abaAtual(), inicio:new Date(ano,mes,21), fim:new Date(ano,mes+1,20)};
  }
  const mesIni=meses.indexOf(m[1]);
  const mesFim=meses.indexOf(m[2]);
  let anoFim=2000+Number(m[3]);
  let anoIni=anoFim;
  if(mesIni>mesFim) anoIni=anoFim-1;
  const inicio=new Date(anoIni,mesIni,21);
  const fim=new Date(anoFim,mesFim,20);
  return {aba,inicio,fim};
}
function datasDoPeriodoFrequencia(nome){
  const p=parsePeriodoFrequencia(nome);
  const datas=[];
...
function formatDateDiaMes(dt){
  return `${String(dt.getDate()).padStart(2,'0')}/${String(dt.getMonth()+1).padStart(2,'0')}`;
}
function getDiaSemanaDate(dt){
  return ['DOM','SEG','TER','QUA','QUI','SEX','SAB'][dt.getDay()];
}
async function carregarPeriodoDoFirebase(datas){
  if(!(fbDb&&fbConnected)) return;
  const meses=new Set(datas.map(d=>`${d.getFullYear()}-${d.getMonth()+1}`));
  for(const k of meses){
    const [ano,mes]=k.split('-').map(Number);
    await carregarMesDoFirebase(ano,mes);
  }
}
function statusToCellByDate(nome,dt){
  const f=EQUIPE.find(x=>x.nome===nome);
  if(!f)return '';
  if(f.turno==='LICENÇA')return 'LICENÇA';

  const diaSem=getDiaSemanaDate(dt);
  const dataBR=formatDateBR(dt);

  if(Array.isArray(f.folgaDom) && f.folgaDom.includes(dataBR)) return 'FOLGA';
  ...
  const dateKey=formatDateISO(dt);
  ...
  if(!rec||!rec.status){
    for(let di=0;di<7;di++){
      ...
      if(wd.getFullYear()===dt.getFullYear() && wd.getMonth()===dt.getMonth() && wd.getDate()===dt.getDate()){
...
async function ensurePlanilha(){
  ...
  const nomeAba=getNomeAbaFrequenciaAtual();
...
async function flushSyncQueue(){
  ...
  syncQueue=[];
  await syncFrequenciaTotal().catch(()=>{});
}
...
async function syncFrequenciaTotal(opts={}){
  if(!gToken){showToast('Conecte o Google primeiro');return;}
  const okPlanilha=await ensurePlanilha();
  if(!okPlanilha)return;

  setSyncStatus('loading','Atualizando frequência...');
  setSheetsStatus('loading','Escrevendo frequência do ciclo...','⏳ AGUARDE...');

  const nomeAba=getNomeAbaFrequenciaAtual();
  const datas=datasDoPeriodoFrequencia(nomeAba);
  const totalDias=datas.length;

  try{
    await carregarPeriodoDoFirebase(datas);
    ...
    const row1=['Mes:', nomeAba, '', 'PERIODO:', ...datas.map(d=>getDiaSemanaDate(d))];
    const row2=['Colaborador:', 'Funcao:', 'Periodo:', 'Status', ...datas.map(d=>formatDateDiaMes(d))];
    const dataRows=sorted.map(f=>{
      const row=[f.nome, f.categoria||'', f.turno||'', ''];
      datas.forEach(dt=>row.push(statusToCellByDate(f.nome,dt)||''));
      return row;
    });
    ...
    if(sheetId!=null && (isNovaAba || opts.forcarFormatacao)){
      await _aplicarFormatacaoAba(sheetId, sorted, totalDias);
    }

    const hora=new Date().toLocaleTimeString('pt-BR');
    setSyncStatus('ok','Frequência atualizada');
    setSheetsStatus('ok',`✅ ${nomeAba} | ${hora}`,'🔗 Abrir Planilha',()=>window.open('https://docs.google.com/spreadsheets/d/'+SHEET_ID,'_blank'));
    ...
    showToast('✅ Frequência '+nomeAba+' atualizada: '+total+' células!');
    lastFullSync=Date.now();
  }catch(e){
    console.error('syncFrequenciaTotal:',e);
    setSyncStatus('err','Erro frequência');
    setSheetsStatus('err','Erro: '+e.message,'📊 TENTAR',()=>syncFrequenciaTotal());
    showToast('❌ Erro: '+e.message);
  }
}

async function atualizarFrequenciaAtual(){
  await syncFrequenciaTotal({forcarFormatacao:false});
}

async function gerarNovaFrequencia(){
  if(!gToken){showToast('⚠️ Conecte o Google Sheets primeiro');return;}
  const base = (typeof ESCALA_ABA!=='undefined' && ESCALA_ABA) ? ESCALA_ABA : getNomeAbaFrequenciaAtual();
  const nome = setNomeAbaFrequenciaAtual(base);
  setSheetsStatus('loading','Gerando nova frequência '+nome+'...','⏳ AGUARDE...');
  await syncFrequenciaTotal({forcarFormatacao:true});
  showToast('✅ Nova frequência gerada: '+nome);
}
...
function _abaAtual(){
  ...
  // Ciclo oficial da escala/frequência: dia 21 de um mês até dia 20 do mês seguinte
  let mesIni, anoIni;
  if(dia >= 21){
    mesIni = mes;
    anoIni = ano;
  ...
  document.getElementById('imp_aba_auto').innerHTML =
    `<span style="color:var(--accent);font-weight:800">📅 ${abaAuto}</span>
     <span style="font-size:10px;color:var(--sub);margin-left:8px">
       (hoje é dia ${hoje.getDate()} — ciclo 21→20)
     </span>`;
...
function buildReport(period){
  ...
  const sheetsBtn = gToken
    ? `<button class="export-btn" onclick="atualizarFrequenciaAtual()" style="background:#1a7340;color:#fff;border-color:#1a7340;font-weight:700;font-size:14px;text-align:center;padding:14px">📊 ATUALIZAR FREQUÊNCIA ATUAL</button>`
      + `<button class="export-btn" onclick="gerarNovaFrequencia()" style="background:#f5c842;color:#111;border-color:#f5c842;font-weight:900;font-size:14px;text-align:center;padding:14px">➕ GERAR NOVA FREQUÊNCIA</button>`
...
// =========================================================
// FREQUÊNCIA POR CICLO 21→20 — FUNÇÕES DE SEGURANÇA
// =========================================================
if (typeof window.atualizarFrequenciaAtual !== 'function') {
  window.atualizarFrequenciaAtual = async function(){
    if (typeof syncFrequenciaTotal === 'function') await syncFrequenciaTotal({forcarFormatacao:false});
    else alert('Função de sincronização da frequência não encontrada.');
  };
}
if (typeof window.gerarNovaFrequencia !== 'function') {
  window.gerarNovaFrequencia = async function(){
    if (typeof syncFrequenciaTotal === 'function') await syncFrequenciaTotal({forcarFormatacao:true});
    else alert('Função de sincronização da frequência não encontrada.');
  };
}
