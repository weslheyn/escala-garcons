  ...
  try{
    const client=google.accounts.oauth2.initTokenClient({
      ...
      callback:async(resp)=>{
        ...
        const okFl=await ensureFreelancePlanilha();
        if(ok){
...
async function gFetch(url,method='GET',body=null){
  ...
  if(!r.ok){
    const msg=(data&&data.error&&(data.error.message||data.error.status)) || data.raw || ('HTTP '+r.status);
    throw new Error(msg);
  }
  return data;
}
...
let SHEET_ID_FL=load('sheet_id_freelance_v1',null);
...
async function ensureFreelancePlanilha(){
  const ano = new Date().getFullYear();
  const nomePlanilha = `Controle de Freelances ${ano} App`;
  const abas = [
    {title:'DADOS_FREELANCE', rows:2000, cols:14},
    {title:'PAGAMENTOS', rows:2000, cols:12},
    {title:'RESUMO', rows:1000, cols:12}
  ];
  const garantirAbas = async ()=>{
    const info=await gFetch(`https://sheets.googleapis.com/v4/spreadsheets/${SHEET_ID_FL}?fields=spreadsheetId,spreadsheetUrl,sheets.properties`);
    if(!(info&&info.spreadsheetId)) throw new Error('Planilha de freelance não localizada');
    const existentes = (info.sheets||[]).map(s=>s.properties.title);
    const faltantes = abas.filter(a=>!existentes.includes(a.title)).map(a=>({addSheet:{properties:{title:a.title,gridProperties:{rowCount:a.rows,columnCount:a.cols}}}}));
    if(faltantes.length){
      await gFetch(`https://sheets.googleapis.com/v4/spreadsheets/${SHEET_ID_FL}:batchUpdate`,'POST',{requests:faltantes});
    }
    const file=await gFetch(`https://www.googleapis.com/drive/v3/files/${SHEET_ID_FL}?fields=id,name,webViewLink`);
    if(!(file&&file.id)) throw new Error('Planilha criada, mas não ficou visível no Drive');
    save('sheet_id_freelance_v1',SHEET_ID_FL);
    if(file.webViewLink) save('sheet_link_freelance_v1',file.webViewLink);
    return file;
  };
  ...
  if(SHEET_ID_FL){
    try{
      ...
      console.warn('Planilha freelance salva não encontrada:', e.message);
      SHEET_ID_FL=null;
      save('sheet_id_freelance_v1',null);
      save('sheet_link_freelance_v1',null);
    }
  }

  try{
    const q = encodeURIComponent(`name='${nomePlanilha}' and mimeType='application/vnd.google-apps.spreadsheet' and trashed=false`);
    const search=await gFetch(`https://www.googleapis.com/drive/v3/files?q=${q}&fields=files(id,name,webViewLink)&pageSize=10`);
    if(search?.files?.length>0){
      SHEET_ID_FL=search.files[0].id;
      save('sheet_id_freelance_v1',SHEET_ID_FL);
      if(search.files[0].webViewLink) save('sheet_link_freelance_v1',search.files[0].webViewLink);
      await garantirAbas();
      showToast('✅ Planilha de freelances conectada automaticamente');
      return true;
    }
  }catch(e){
    console.warn('Busca drive freelance:', e.message);
  }

  try{
    const r=await gFetch('https://sheets.googleapis.com/v4/spreadsheets','POST',{
      properties:{title:nomePlanilha},
      sheets: abas.map(a=>({properties:{title:a.title,gridProperties:{rowCount:a.rows,columnCount:a.cols}}}))
    });
    if(!r||!r.spreadsheetId) throw new Error('API não retornou spreadsheetId');
    SHEET_ID_FL=r.spreadsheetId;
    save('sheet_id_freelance_v1',SHEET_ID_FL);
    const file=await gFetch(`https://www.googleapis.com/drive/v3/files/${SHEET_ID_FL}?fields=id,name,webViewLink`);
    if(!(file&&file.id)) throw new Error('A API criou a planilha, mas ela não apareceu no Drive');
    if(file.webViewLink) save('sheet_link_freelance_v1',file.webViewLink);
    await garantirAbas();
    showToast('✅ Planilha de freelances criada no Drive');
    return true;
  }catch(e){
    console.warn('Erro ao criar planilha de freelance:', e.message);
    showToast('❌ Falha ao criar a planilha de freelances: '+e.message);
    return false;
  }
}

async function syncFreelanceSheets(){
  if(!gToken||!SHEET_ID_FL) return;
  try{
    const fls = Object.values(_flCache||{}).sort((a,b)=>(a.nome||'').localeCompare(b.nome||''));
    const dias=[['ID_PACOTE','NOME','FUNCAO','DATA_INICIO','DIA_PACOTE','DATA_REAL','STATUS','VALOR_DIARIA','VALOR_PAGO_DIA','PAGO','FECHAMENTO_PREVISTO','DECISAO_7_DIA','PACOTE_STATUS','OBS']];
    const pagamentos=[['ID_PACOTE','NOME','FUNCAO','TIPO','DATA','VALOR','REFERENCIA','STATUS','FECHAMENTO_PREVISTO','DECISAO_7_DIA','USUARIO','OBS']];
    const resumo=[['ID_PACOTE','NOME','FUNCAO','DATA_INICIO','PRESENTES','CAMPOS_USADOS','VALOR_DIA','TOTAL_TRABALHADO','TOTAL_PAGO','SALDO','FECHAMENTO_PREVISTO','STATUS']];
    fls.forEach(fl=>{
      const r=_flResumoPagamento(fl);
      const mapa=_flPagamentoPorSlot(fl);
      const totalCampos=_flTotalCampos(fl.id);
      const finalInfo=_flPagamentoFinalInfo(fl);
      const decision = fl.fechamentoModo || (finalInfo?.estado==='aguardando8' ? 'estender8' : (finalInfo ? 'quitar7' : ''));
      _flSlots(fl.id).forEach(s=>{
        const p=mapa[s.ordem]||{};
        dias.push([
          fl.id, fl.nome||'', fl.funcao||'', fl.dataIni||'', s.ordem, s.data||'', s.status||'', Number(fl.valor)||0,
          Number(p.pago||0), p.status||'', fl.fechamentoPrevisto||7, decision, (r.worked>=_flTotalDiarias(fl)?'fechado':'em_andamento'), ''
        ]);
      });
      _flPayments(fl.id).forEach(pay=>{
        pagamentos.push([
          fl.id, fl.nome||'', fl.funcao||'', pay.tipo||'', pay.data||'', Number(pay.valor||0), pay.referencia||'', 'pago',
          fl.fechamentoPrevisto||7, decision, pay.user||'', pay.obs||''
        ]);
      });
      resumo.push([
        fl.id, fl.nome||'', fl.funcao||'', fl.dataIni||'', r.worked, _flFilledCount(fl.id), Number(fl.valor)||0,
        r.totalTrabalhado, r.totalPago, r.saldo, fl.fechamentoPrevisto||7,
        r.saldo<=0 && r.worked>=7 ? 'quitado' : (finalInfo?.estado==='aguardando8' ? 'aguardando_8_dia' : (r.worked>=7 ? 'pronto_para_fechar' : 'em_andamento'))
      ]);
    });
    await gFetch(`https://sheets.googleapis.com/v4/spreadsheets/${SHEET_ID_FL}/values:batchUpdate`,'POST',{
      valueInputOption:'USER_ENTERED',
      data:[
        {range:'DADOS_FREELANCE!A1:N', values:dias},
        {range:'PAGAMENTOS!A1:L', values:pagamentos},
        {range:'RESUMO!A1:L', values:resumo}
      ]
    });
  }catch(e){ console.warn('syncFreelanceSheets:', e.message); }
}

function triggerFreelanceSheetsSync(){
  if(!gToken) return;
  clearTimeout(window._freelanceSheetsTimer);
  window._freelanceSheetsTimer=setTimeout(async ()=>{
    try{
      if(!SHEET_ID_FL){
        const ok = await ensureFreelancePlanilha();
        if(!ok) return;
      }
      await syncFreelanceSheets();
    }catch(e){
      console.warn('freelance sheet auto-sync:', e.message);
      showToast('❌ Falha ao sincronizar freelances: '+e.message);
    }
  },2500);
}
...
function startAutoSync(){
  ...
  if(gToken&&SHEET_ID_FL){
    syncFreelanceSheets().catch(()=>{});
  }
...
function _flTotalDiarias(fl){
  const bruto = parseInt(fl?.metaPresencas || fl?.totalDiarias || fl?.dias || 8) || 8;
  // Meta fixa de 8 presenças. Faltas/folgas não encerram o pacote.
  return Math.max(8, Math.min(8, bruto));
}
function _flTotalCampos(flId){
  const fl = _flCache[flId] || {};
  const meta = _flTotalDiarias(fl);
  const raw = Object.values(_flSlotsCache[flId] || {}).filter(Boolean);
  const preenchidos = raw.filter(s=>s && s.status).length;
  const presentes = raw.filter(s=>s && s.status==='presente').length;
  const faltasFolgas = raw.filter(s=>s && (s.status==='falta' || s.status==='folga')).length;
  const calculado = Math.max(meta, meta + faltasFolgas, preenchidos + Math.max(0, meta - presentes));
  const salvo = parseInt(fl.totalCampos || 0) || 0;
  return Math.max(meta, Math.min(31, Math.max(calculado, salvo)));
}
function _flEnsureSlots(flId, total=8){
  total = Math.max(1, Math.min(31, parseInt(total)||8));
...
function _flPersistAllSlots(flId){
  if(!(fbDb&&fbConnected)) return;
  const payload={};
  Object.values(_flSlotsCache[flId]||{})
    .filter(Boolean)
    .sort((a,b)=>(a.ordem||0)-(b.ordem||0))
    .forEach(s=>{ payload[s.ordem]=s; });
  fbDb.ref('freelances_slots/'+flId).set(payload).catch(e=>console.warn('FL slots sync:',e));
}
function _flExpandSlotsIfNeeded(flId){
  const fl = _flCache[flId] || {};
  const needed = _flTotalCampos(flId);
  _flEnsureSlots(flId, needed);
  if(fl.totalCampos !== needed){
    fl.totalCampos = needed;
    if(fl.id) _flCache[flId] = fl;
    _salvarFlFirebase(fl);
  }
  _flPersistAllSlots(flId);
}
function _flSlots(flId){
  const total = _flTotalCampos(flId);
...
function _flAtualizarFechamento(flId){
  ...
  const fechado = _flWorkedCount(flId) >= _flTotalDiarias(fl);
...
function _flPagamentoFinalInfo(fl){
  const resumo = _flResumoPagamento(fl);
  const worked = _flWorkedCount(fl.id);
  const previsto = Number(fl.fechamentoPrevisto||7) || 7;
  const modo = fl.fechamentoModo || '';
  if(resumo.saldo<=0 || worked<previsto) return null;
  if(modo==='estender8'){
    if(worked<8) return {estado:'aguardando8', valor:resumo.saldo, worked, previsto:8, label:'Aguardando 8º dia'};
    return {estado:'quitar8', valor:resumo.saldo, worked, previsto:8, label:'Quitar no 8º dia'};
  }
  return {estado:'decidir7', valor:resumo.saldo, worked, previsto:7, label:'Fechar no 7º dia'};
}
function _flQuitacaoPendente(fl){
  return !!_flPagamentoFinalInfo(fl);
}
...
async function _carregarFreelancesFirebase(){
  ...
  try{
    ...
    Object.values(_flCache).forEach(fl=>{
      if(!fl || !fl.id) return;
      const totalPadrao = _flTotalDiarias(fl);
      const mudouPacote = Number(fl.dias||fl.totalDiarias||0) !== totalPadrao;
      fl.dias = totalPadrao;
      fl.totalDiarias = totalPadrao;
      if(!fl.fechamentoPrevisto) fl.fechamentoPrevisto = 7;
      if(!fl.totalCampos || Number(fl.totalCampos) < totalPadrao) fl.totalCampos = totalPadrao;
      _flEnsureSlots(fl.id, fl.totalCampos || totalPadrao);
...
function _flStatus(fl){
  ...
  const encerrado = _flWorkedCount(fl.id) >= _flTotalDiarias(fl);
...
function _flPassaFiltro(fl, filtro){
  ...
  if(filtro==='hoje') return datas.some(d=>d.toDateString()===hoje.toDateString()) || _flWorkedCount(fl.id)<_flTotalDiarias(fl);
  if(filtro==='semana') return datas.some(d=>d>=seg&&d<=dom) || _flWorkedCount(fl.id)<_flTotalDiarias(fl);
  if(filtro==='mes') return datas.some(d=>d.getMonth()===hoje.getMonth()&&d.getFullYear()===hoje.getFullYear()) || _flWorkedCount(fl.id)<_flTotalDiarias(fl);
  return true;
}
...
function _renderFlLista(el, fls){
  ...
  el.innerHTML = fls.map(fl=>{
    const resumo = _flResumoPagamento(fl);
    const total = _flTotalCampos(fl.id);
    const campos = _flTotalCampos(fl.id);
    ...
    return `<div style="background:var(--s2);border-radius:14px;padding:13px 15px;border:1px solid var(--border);border-left:3px solid ${bordaCor};margin:0 14px 10px;box-shadow:0 2px 8px #00000020">
      <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:8px;gap:8px">
        <div style="flex:1;min-width:0">
          ...
          <div style="font-size:10px;color:var(--sub);margin-top:2px">${fl.funcao} · ${resumo.worked}/${total} presenças · ${filled}/${campos} campos usados</div>
        </div>
      ...
      <div style="font-size:10px;color:var(--sub)">${primeiro?'💰 Falta registrar o pagamento do 1º dia. ':''}${(_flPagamentoFinalInfo(fl)?.estado==='aguardando8')?'⏳ Fechamento estendido para o 8º dia. ':''}${(quitacao&&_flPagamentoFinalInfo(fl)?.estado!=='aguardando8')?'💳 Fechamento pendente no 7º/8º dia. ':''}${fl.auth?('Autorizado por '+fl.auth):''}</div>
    </div>`;
  }).join('');
}
...
function _renderFlPagamento(el, fls, tipo){
  ...
  const total = lista.reduce((s,fl)=>{
    const resumo = _flResumoPagamento(fl);
    const finalInfo = _flPagamentoFinalInfo(fl);
    return s + (tipo==='primeiro' ? Math.min(resumo.valorDia, resumo.saldo||resumo.valorDia) : Number(finalInfo?.valor||resumo.saldo||0));
  }, 0);
  const cor = tipo==='primeiro' ? '#27ae60' : '#f5c842';
  const titulo = tipo==='primeiro' ? '💰 PAGAMENTO DO 1º DIA PENDENTE' : '💳 FECHAMENTO 7º / 8º DIA';
  if(!lista.length){
  ...
  el.innerHTML = `<div style="background:${cor}18;border:1px solid ${cor}44;border-radius:12px;padding:12px 16px;margin:6px 14px 12px;text-align:center"><div style="font-size:10px;font-weight:700;color:${cor};text-transform:uppercase;letter-spacing:.5px;margin-bottom:4px">${titulo}</div><div style="font-size:28px;font-weight:800;color:${cor};font-family:'Barlow Condensed',sans-serif">R$ ${_fmtMoney(total)}</div><div style="font-size:10px;color:var(--sub);margin-top:2px">${lista.length} freelance(s)</div></div>`+
    lista.map(fl=>{
      const resumo = _flResumoPagamento(fl);
      const finalInfo = _flPagamentoFinalInfo(fl);
      const valor = tipo==='primeiro' ? Math.min(resumo.valorDia, resumo.saldo||resumo.valorDia) : Number(finalInfo?.valor||resumo.saldo||0);
      const sub = tipo==='primeiro' ? `${fl.funcao} · ${_flWorkedCount(fl.id)}/${_flTotalDiarias(fl)} presenças` : `${fl.funcao} · ${finalInfo?.label||'Fechamento pendente'}`;
      return `<div style="background:var(--s2);border-radius:12px;padding:12px 14px;border:1px solid ${cor}33;margin:0 14px 8px;display:flex;align-items:center;gap:10px">
        <div style="flex:1"><div style="font-weight:700;font-size:14px">${fl.nome}</div><div style="font-size:10px;color:var(--sub)">${sub}</div></div>
...
function _flResumoGeral(fls){
  return fls.reduce((acc,fl)=>{
    ...
    if((_flWorkedCount(fl.id) >= _flTotalDiarias(fl)) && r.saldo<=0) acc.quitados += 1;
    if(_flWorkedCount(fl.id) < _flTotalDiarias(fl)) acc.emAndamento += 1;
    return acc;
...
function _renderFlFinanceiro(el, fls){
  ...
  const lista = fls.map(fl=>{
    const r = _flResumoPagamento(fl);
    const fechado = _flWorkedCount(fl.id) >= _flTotalDiarias(fl);
...
function _renderFlFrequencia(el, fls){
  if(!fls.length){ el.innerHTML='<div class="empty">Nenhum freelance encontrado.</div>'; return; }
  el.innerHTML = fls.map(fl=>{
    const total = _flTotalDiarias(fl);
    const payMap = _flPagamentoPorSlot(fl);
    const slotsHtml = _flSlots(fl.id).map(s=>{
...
function toggleFlSlot(flId, ordem, btn){
  ...
  _renderAlertas();
  buildFlContent();
  showToast((prox==='falta'||prox==='folga') ? '✅ '+prox.charAt(0).toUpperCase()+prox.slice(1)+' — campo extra liberado para completar 8 presenças' : (prox ? '✅ '+prox.charAt(0).toUpperCase()+prox.slice(1) : '— Registro removido'));
}
...
function openFlPayModal(flId, foco=''){
  ...
  const finalInfo = _flPagamentoFinalInfo(fl);
  const worked = _flWorkedCount(fl.id);
  const eventos = _flPayments(fl.id).map(p=>`<div style="display:flex;align-items:center;gap:8px;padding:8px 10px;border-bottom:1px solid var(--border)"><div style="flex:1"><div style="font-size:12px;font-weight:700">${p.tipo==='primeiro-dia'?'💰 1º dia':p.tipo==='quitacao'?'💳 quitação':p.tipo==='saldo-7'?'💳 saldo 7º dia':p.tipo==='saldo-8'?'💳 saldo 8º dia':'🟣 antecipação'}</div><div style="font-size:10px;color:var(--sub)">${_fmtData(p.data)}${p.obs?(' · '+p.obs):''}</div></div><div style="font-size:13px;font-weight:800;color:#27ae60">R$ ${_fmtMoney(p.valor)}</div><button onclick="excluirFlPagamento('${fl.id}','${p.id}')" style="background:none;border:1px solid #e74c3c44;color:#e74c3c;border-radius:8px;padding:4px 8px;font-size:11px;cursor:pointer">🗑️</button></div>`).join('') || `<div style="padding:14px 10px;color:var(--sub);text-align:center;font-size:12px">Nenhum pagamento registrado.</div>`;
  const podePrimeiro = _flPrimeiroPagamentoPendente(fl);
  const quitarTxt = finalInfo ? (finalInfo.estado==='quitar8' ? `💳 Quitar saldo do 8º dia (R$ ${_fmtMoney(finalInfo.valor)})` : `💳 Quitar saldo no 7º dia (R$ ${_fmtMoney(finalInfo.valor)})`) : `💳 Quitar restante (R$ ${_fmtMoney(resumo.saldo)})`;
  const blocoFechamento = finalInfo ? `
    <div style="background:${finalInfo.estado==='aguardando8'?'#2980b918':'#f5c84218'};border:1px solid ${finalInfo.estado==='aguardando8'?'#2980b944':'#f5c84244'};border-radius:10px;padding:10px 12px;margin-bottom:12px">
      <div style="font-size:10px;font-weight:700;color:${finalInfo.estado==='aguardando8'?'#2980b9':'#f5c842'};text-transform:uppercase;letter-spacing:.4px">Fechamento financeiro</div>
      <div style="font-size:13px;font-weight:700;margin-top:4px">${finalInfo.label}</div>
      <div style="font-size:10px;color:var(--sub);margin-top:2px">${worked} presença(s) confirmada(s) · saldo atual R$ ${_fmtMoney(finalInfo.valor)}</div>
      ${worked>=7 ? `<div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:10px">
        <button class="export-btn" onclick="definirFechamentoFreelance('${fl.id}','quitar7')" style="margin-bottom:0;flex:1;min-width:150px;border-color:#f5c84244;color:#f5c842">💳 Fechar no 7º dia</button>
        <button class="export-btn" onclick="definirFechamentoFreelance('${fl.id}','estender8')" style="margin-bottom:0;flex:1;min-width:150px;border-color:#2980b944;color:#2980b9">➡️ Estender para o 8º dia</button>
      </div>`:''}
    </div>` : '';
  document.getElementById('flFreqTitulo').textContent='💳 '+fl.nome;
  document.getElementById('flFreqContent').innerHTML=`<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:6px;margin-bottom:12px"><div style="background:#27ae6018;border-radius:9px;padding:8px;text-align:center"><div style="font-size:18px;font-weight:800;color:#27ae60">R$ ${_fmtMoney(resumo.totalTrabalhado)}</div><div style="font-size:9px;color:var(--sub)">TRABALHADO</div></div><div style="background:#2980b918;border-radius:9px;padding:8px;text-align:center"><div style="font-size:18px;font-weight:800;color:#2980b9">R$ ${_fmtMoney(resumo.totalPago)}</div><div style="font-size:9px;color:var(--sub)">PAGO</div></div><div style="background:#f5c84218;border-radius:9px;padding:8px;text-align:center"><div style="font-size:18px;font-weight:800;color:#f5c842">R$ ${_fmtMoney(resumo.saldo)}</div><div style="font-size:9px;color:var(--sub)">SALDO</div></div></div>
    ...
    <div style="display:grid;grid-template-columns:1fr;gap:8px;margin-bottom:12px">
      ...
      <button class="export-btn" onclick="registrarFlPagamentoRapido('${fl.id}','quitacao')" style="margin-bottom:0;${foco==='ultimo'?'border-color:#f5c842;color:#f5c842':''}">${quitarTxt}</button>
    </div>
...
function definirFechamentoFreelance(flId, modo){
  const fl = _flCache[flId]; if(!fl) return;
  fl.fechamentoPrevisto = 7;
  fl.fechamentoModo = modo;
  if(modo==='quitar7' && _flWorkedCount(flId)>=7){
    showToast('💳 Fechamento mantido no 7º dia');
  }else if(modo==='estender8'){
    showToast('➡️ Fechamento estendido para o 8º dia');
  }
  _salvarFlFirebase(fl);
  triggerFreelanceSheetsSync();
  _renderAlertas();
...
function registrarFlPagamentoRapido(flId, tipo){
  ...
  if(tipo==='primeiro-dia'){
    ...
    valor = resumo.saldo;
    if(finalInfo?.estado==='quitar8'){
      tipo = 'saldo-8';
      fl.fechamentoModo = 'estender8';
      referencia = 'quitação 8º dia';
    }else{
      tipo = 'saldo-7';
      fl.fechamentoModo = 'quitar7';
      referencia = 'quitação 7º dia';
    }
  }else{
    ...
    referencia = 'antecipação';
  }
  const pay = {id:'pay_'+Date.now()+'_'+Math.random().toString(36).slice(2,5), data:_isoHoje(), valor:+valor.toFixed(2), tipo, referencia, obs, ts:Date.now()};
  _saveFlPayment(flId, pay);
  if(tipo==='saldo-7' || tipo==='saldo-8'){ fl.fechamentoModo='quitado'; }
  _salvarFlFirebase(fl);
  _flAtualizarFechamento(flId);
  triggerFreelanceSheetsSync();
  _renderAlertas();
...
FECHAMENTO: ${_flPagamentoFinalInfo(fl)?.label || (fl.fechamentoModo==='quitado'?'Quitado':'7º dia')}
...
function salvarFreelance(){
  ...
  const fl = {id, nome, funcao, dataIni, dias, totalDiarias:dias, metaPresencas:dias, totalCampos:Math.max(8, Number(_flCache[idExist]?.totalCampos||dias)||8), valor, auth, fechamentoPrevisto:7, fechamentoModo:(_flCache[idExist]?.fechamentoModo||''), criadoEm:idExist?(_flCache[idExist]?.criadoEm||Date.now()):Date.now()};
  _flCache[id]=fl;
  ...
  _renderAlertas();
  buildFlContent();
  showToast('✅ Freelance salvo com meta de '+dias+' presenças');
}
