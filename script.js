...
const isFolgaSemanal=(f,diaSem)=>!!f&&(String(f.folga||'').toUpperCase()===diaSem||String(f.folga2||'').toUpperCase()===diaSem);

// Cache reforçado das folgas de domingo vindo diretamente do Firebase/escala oficial.
// Isso evita que a tela de frequência use uma equipe antiga salva no navegador.
let FOLGA_DOM_OFICIAL_MAP = {};
function _registrarFolgaDomNome(nome, folgaDom, anoBase){
  const n = (typeof _normNome === 'function' ? _normNome(nome) : String(nome||'').toUpperCase().trim());
  if(!n) return;
  const arr = _normalizarListaFolgaDom(folgaDom, anoBase);
  if(!arr.length) return;
  if(!FOLGA_DOM_OFICIAL_MAP[n]) FOLGA_DOM_OFICIAL_MAP[n] = [];
  FOLGA_DOM_OFICIAL_MAP[n] = [...new Set([...(FOLGA_DOM_OFICIAL_MAP[n]||[]), ...arr])];
}
function _registrarFolgasDomOficiais(payload){
  if(!payload) return;
  const anoBase = new Date().getFullYear();
  const funcs = Array.isArray(payload.funcionarios) ? payload.funcionarios : [];
  funcs.forEach(f=>_registrarFolgaDomNome(f.nome, f.folgaDom || f.FOLGA_DOM || f.folga_dom, anoBase));
  const colabs = Array.isArray(payload.colaboradores) ? payload.colaboradores : [];
  colabs.forEach(r=>{
    if(Array.isArray(r)) _registrarFolgaDomNome(r[2], r[5], anoBase);
    else if(r && typeof r==='object') _registrarFolgaDomNome(r.nome||r.COLABORADOR||r.colaborador, r.folgaDom||r.FOLGA_DOM||r.folga_dom, anoBase);
  });
}
function _listaFolgaDomFuncionario(f, anoBase){
  const n = (typeof _normNome === 'function' ? _normNome(f && f.nome) : String((f&&f.nome)||'').toUpperCase().trim());
  const direto = _normalizarListaFolgaDom(f && f.folgaDom, anoBase);
  const oficial = _normalizarListaFolgaDom(FOLGA_DOM_OFICIAL_MAP[n] || [], anoBase);
  return [...new Set([...direto, ...oficial].filter(Boolean))];
}
// Folga real da escala importada.
// Regra crítica: DOMINGO não deve usar a folga semanal normal quando há tabela de "Domingo do mês".
// A fonte da verdade é f.folgaDom, importada da planilha oficial/Drive.
function _fmtDataBRFolga(dt){
  if(!dt) return '';
  const d = dt instanceof Date ? dt : new Date(dt);
  if(isNaN(d)) return '';
  return String(d.getDate()).padStart(2,'0')+'/'+String(d.getMonth()+1).padStart(2,'0')+'/'+d.getFullYear();
}
function _fmtDataCurtaFolga(dt){
  const full=_fmtDataBRFolga(dt);
  return full ? full.slice(0,5) : '';
}
function _normDataFolgaValor(v, anoBase){
  if(!v) return '';
  if(v instanceof Date && !isNaN(v)) return _fmtDataBRFolga(v);
  let s=String(v).trim();
  if(!s) return '';
  ...
  const curta=s.match(/^(\d{1,2})\/(\d{1,2})$/);
  if(curta && anoBase) return `${curta[1].padStart(2,'0')}/${curta[2].padStart(2,'0')}/${anoBase}`;
  return s;
}
function _normalizarListaFolgaDom(valor, anoBase){
  let bruto=[];
  if(Array.isArray(valor)) bruto=valor;
  else if(valor && typeof valor==='object') bruto=Object.values(valor);
  else if(valor) bruto=String(valor).split(/[,;|\n]+/);

  const out=[];
  bruto.forEach(v=>{
    if(v instanceof Date && !isNaN(v)){
      out.push(_fmtDataBRFolga(v));
      return;
    }
    const txt=String(v||'').trim();
    if(!txt || /^FOLGA$/i.test(txt) || /^LICEN/i.test(txt)) return;

    const iso=[...txt.matchAll(/(\d{4})-(\d{2})-(\d{2})/g)];
    iso.forEach(m=>out.push(`${m[3]}/${m[2]}/${m[1]}`));

    const completas=[...txt.matchAll(/(\d{1,2})\/(\d{1,2})\/(\d{4})/g)];
    completas.forEach(m=>out.push(`${m[1].padStart(2,'0')}/${m[2].padStart(2,'0')}/${m[3]}`));

    // Aceita a estrutura real do mural/Firebase: chaves curtas como 03/05.
    if(!iso.length && !completas.length){
      const curtas=[...txt.matchAll(/(^|\D)(\d{1,2})\/(\d{1,2})(?!\/)($|\D)/g)];
      curtas.forEach(m=>{
        const dd=String(m[2]).padStart(2,'0');
        const mm=String(m[3]).padStart(2,'0');
        out.push(anoBase ? `${dd}/${mm}/${anoBase}` : `${dd}/${mm}`);
      });
    }
  });
  return [...new Set(out.filter(Boolean))];
}
function _funcionarioFolgaDomingoNaData(f, dataObj){
  const dt=dataObj instanceof Date ? dataObj : new Date(dataObj);
  if(!f || isNaN(dt)) return false;
  const ano=dt.getFullYear();
  const alvoFull=_fmtDataBRFolga(dt);
  const alvoCurto=_fmtDataCurtaFolga(dt);
  const arr=_listaFolgaDomFuncionario(f, ano);
  if(arr.length){
    return arr.some(v=>{
      const full=_normDataFolgaValor(v, ano);
      return full===alvoFull || String(v).slice(0,5)===alvoCurto;
    });
  }
  return String(f.folga||'').toUpperCase()==='DOM'||String(f.folga2||'').toUpperCase()==='DOM';
}
function isFolgaEscala(f,diaSem,dataObj){
  if(!f) return false;
  const ds=String(diaSem||'').toUpperCase();
  if(ds==='DOM') return _funcionarioFolgaDomingoNaData(f,dataObj);
  return isFolgaSemanal(f,ds);
}
const CATS=[
...
const GCLIENT_ID='429328314952-0oc4osbaqinlsoghp869nvv3v2hf8p43.apps.googleusercontent.com';
...
function _equipeSnapshotPayload(meta){
  return {
    funcionarios: (EQUIPE||[]).map(f=>({...f})),
    total: (EQUIPE||[]).length,
    versao: 'eq_'+Date.now(),
    origem: meta && meta.origem ? meta.origem : (ESCALA_ABA || ''),
    atualizadoEm: Date.now(),
    atualizadoEmTexto: new Date().toLocaleString('pt-BR')
  };
}

async function publicarEquipeOficial(meta){
  if(!(fbDb && fbConnected)) return false;
  const payload = _equipeSnapshotPayload(meta || {});
  await fbDb.ref(EQUIPE_OFICIAL_PATH).set(payload);
  await fbDb.ref('escala_oficial/ultima_equipe_app').set(payload);
  return true;
}

function aplicarEquipeCentral(payload, opts){
  if(!payload || !Array.isArray(payload.funcionarios) || !payload.funcionarios.length) return;
  // V42: sempre reaplica a equipe oficial recebida do Firebase.
  // Antes, se a versão local fosse igual, o app ignorava o payload e mantinha
  // equipe antiga no navegador sem folgaDom atualizada.
  _registrarFolgasDomOficiais(payload);
  _aplicandoEquipeRemota = true;
  EQUIPE = payload.funcionarios.map(f=>({...f, folgaDom: _normalizarListaFolgaDom(f.folgaDom)}));
  save('equipe', EQUIPE);
  save('equipe_version', payload.versao || ('eq_remote_'+Date.now()));
  _aplicandoEquipeRemota = false;
  const totalPill = document.getElementById('totalPill');
  if(totalPill) totalPill.textContent = EQUIPE.length + ' TOTAL';
  const bar=document.getElementById('importStatusBar');
  if(bar){
    const origem = payload.origem ? ' — '+payload.origem : '';
    const dataTxt = payload.atualizadoEmTexto || (payload.atualizadoEm ? new Date(payload.atualizadoEm).toLocaleString('pt-BR') : '');
    bar.style.display='block';
    bar.innerHTML=`<div style="display:flex;align-items:center;gap:8px;color:var(--green)"><span>✅</span><div><span style="font-weight:700">Equipe oficial sincronizada${origem} — ${EQUIPE.length} funcionários</span><span style="color:var(--sub);font-size:10px;margin-left:6px">${dataTxt}</span></div><button onclick="abrirImportarEscala()" style="margin-left:auto;background:none;border:1px solid var(--border);color:var(--accent);border-radius:6px;padding:3px 10px;font-size:10px;font-weight:700;cursor:pointer">↻ Reimportar</button></div>`;
  }
  buildMain(); refreshPendBadge(); buildSummary();
  if(document.getElementById('equipePanel') && document.getElementById('equipePanel').classList.contains('open')) buildEquipeContent('TODOS');
  if(document.getElementById('mapaPanel') && document.getElementById('mapaPanel').classList.contains('open')) renderMapa();
}
...
function _turnoPorGrupoEscala(grupo, horario, funcao, status){
  ...
  if(g.includes('INTERCALADO') || g.includes('LIDERANCA') || g.includes('VASTO')) return 'INTERCALADO';
  if(g.includes('ADMINISTRATIVO') || g.includes('MANUTENCAO') || g.includes('NUTRICAO')) return 'ADMINISTRATIVO';
  try { return _detectTurno(String(horario||''), String(funcao||'')); } catch(e){ return g || 'INTERCALADO'; }
}
function _colaboradoresTabelaParaEquipe(rows){
  ...
  rows.forEach(row=>{
    ...
    const folgaPadrao = String(row[4]||'').trim().toUpperCase();
    const folgaDomRaw = _normalizarDataFolgaDom(row[5]||'');
    const horario = String(row[6]||'').trim();
    const ativo = String(row[7]||'SIM').trim().toUpperCase();
    const status = String(row[9]||'').trim().toUpperCase();
    if(!nome || ativo==='NAO' || ativo==='NÃO') return;
    const key = (typeof _normNome==='function') ? _normNome(nome) : nome.toUpperCase();
    if(vistos.has(key)) return;
    vistos.add(key);
    let ini='', fim='';
    try { const p=_parseHorario(horario); ini=p.ini||''; fim=p.fim||''; } catch(e){}
    let categoria = funcao || grupo || 'FUNCIONÁRIO';
    try { categoria = _mapCategoria(funcao); } catch(e){}
    const turno = _turnoPorGrupoEscala(grupo, horario, funcao, status);
...
function aplicarEscalaOficialFirebase(payload, opts){
  if(!payload) return;
  _registrarFolgasDomOficiais(payload);
  const rows = payload.colaboradores || payload.funcionarios || [];
  const equipe = _colaboradoresTabelaParaEquipe(rows);
  if(!equipe.length) return;
  const versao = 'escala_oficial_' + (payload.atualizadoEm || payload.nomeAbaEscala || payload.nomePlanilha || 'ultima');
  const versaoAtual = load('equipe_version', null);
  if(opts && opts.silent && versaoAtual === versao) return;
  aplicarEquipeCentral({
    funcionarios:equipe,
    total:equipe.length,
    versao,
    origem:payload.nomeAbaEscala || payload.origem || 'ESCALA OFICIAL',
    atualizadoEm: payload.atualizadoEm ? (Date.parse(payload.atualizadoEm)||Date.now()) : Date.now(),
    atualizadoEmTexto: payload.atualizadoEm ? new Date(payload.atualizadoEm).toLocaleString('pt-BR') : new Date().toLocaleString('pt-BR')
  }, opts || {});
}

function iniciarListenerEquipeOficial(){
  if(!(fbDb && fbConnected) || _equipeCentralListenerActive) return;
  _equipeCentralListenerActive = true;
...
const EQUIPE_VERSION = 'v20260503_v41_folga_dom_real';
...
let EQUIPE = (()=>{
  ...
  // Nunca derruba a equipe oficial importada só por versão de código.
  // Se houver equipe salva do Drive/Firebase, ela é preservada e normalizada.
  if(savedEquipe && Array.isArray(savedEquipe) && savedEquipe.length >= 20){
    const eq = savedEquipe.map(f=>({...f, folgaDom:_normalizarListaFolgaDom(f.folgaDom)}));
    _registrarFolgasDomOficiais({funcionarios:eq});
    return eq;
  }
  const eqDefault = EQUIPE_DEFAULT.map(f=>({...f, folgaDom:_normalizarListaFolgaDom(f.folgaDom)}));
  save('equipe', eqDefault);
  save('equipe_version', EQUIPE_VERSION);
  _registrarFolgasDomOficiais({funcionarios:eqDefault});
  return eqDefault;
})();
...
function getPendencias(){
  ...
  EQUIPE.forEach(f=>{
    ...
    for(let d=0;d<=todayIdx-1;d++){
      if(isFolgaEscala(f,DAYS[d],WEEK_DATES[d]))continue;
      const r=getRec(d,f.nome);
  ...
  try{
    const client=google.accounts.oauth2.initTokenClient({
      ...
      callback:async(resp)=>{
        ...
        const CONTAS_PERMITIDAS=['weslheynsd@gmail.com','gerencia.barra@cocobambu.com'];
        let emailConta='';
        try{
...
async function aplicarImportacao(){
  ...
  EQUIPE = lidos.map(f => {
    ...
    return {
      ...
      folgaDom:  _normalizarListaFolgaDom(f.folgaDom) || undefined,
    };
  });
  ...
  save('equipe', EQUIPE);
  const novaVersaoEquipe = 'esc_'+ts;
  save('equipe_version', novaVersaoEquipe);

  // Publica a equipe no Firebase oficial para todos os dispositivos.
  // Sem esta publicação, outros aparelhos e recarregamentos continuam com a equipe antiga.
  let publicadoFirebase = false;
  if(fbDb && fbConnected){
    try{
      publicadoFirebase = await publicarEquipeOficial({origem: aba});
    }catch(e){
      console.error('Erro ao publicar equipe oficial:', e);
      publicadoFirebase = false;
    }
  }
  if(!publicadoFirebase){
    showToast('⚠️ Escala aplicada neste aparelho, mas NÃO sincronizou no Firebase. Verifique internet/Firebase e importe novamente.');
  }
...
async function saveFuncModal(){
  ...
  if(fbDb&&fbConnected&&!_aplicandoEquipeRemota) await publicarEquipeOficial({origem:'edição manual no app'}).catch(()=>{});
...
async function delFunc(id){
  ...
  if(fbDb&&fbConnected&&!_aplicandoEquipeRemota) await publicarEquipeOficial({origem:'remoção manual no app'}).catch(()=>{});
  if(gToken)await pushEquipeToSheets().catch(()=>{});
  buildEquipeContent('TODOS');
}
...
function buildReport(period){
  ...
  days.forEach(d=>{
    ...
    EQUIPE.forEach(f=>{
      ...
      if(isFolgaEscala(f,DAYS[d],WEEK_DATES[d])){grupos.folga.push(f.nome);return;}
      const r=getRec(d,f.nome);
...
async function exportToSheets(){
  ...
  const dataRows=sorted.map(f=>{
    ...
    for(let d=0;d<6;d++){
      ...
      if(isFolgaEscala(f,DAYS[d],WEEK_DATES[d])){row.push('FOLGA');continue;}
      const r=getRec(d,f.nome);
...
function setMapaSetor(s){
  _mapaSetor=s;
  ['garcom','cumim','bar','gerente','maitre','recepcao','asg','encantadora','outros'].forEach(x=>{
...
function _getFuncsSetor(setor){
  const cat = f => _normTxt(f.categoria);
  const grp = f => _normTxt(f.grupoEscala || f.grupo || f.setor);
  const isGarcom = f => ['GARCOM','CHEFE DE FILA'].includes(cat(f)) || grp(f).includes('GARCOM');
  const isCumim = f => cat(f)==='CUMIM' || grp(f).includes('CUMIM');
  const isBar = f => ['BARMAN','AUXILIAR DE BAR','COPEIRO'].includes(cat(f)) || grp(f).includes('BAR');
  const isGerente = f => cat(f).includes('GERENTE') || grp(f).includes('GERENCIA');
  const isMaitre = f => cat(f).includes('MAITRE') || grp(f).includes('MAITRE');
  const isRecepcao = f => cat(f).includes('RECEPCIONISTA') || grp(f).includes('RECEP');
  const isAsg = f => cat(f).includes('ASG') || cat(f).includes('LIMPEZA') || grp(f).includes('GOVERNANCA') || grp(f).includes('ASG');
  const isEncantadora = f => cat(f).includes('ENCANTADORA') || grp(f).includes('BRINQUEDOTECA');
  const cats = {
    garcom: isGarcom, cumim: isCumim, bar: isBar, gerente: isGerente, maitre: isMaitre, recepcao: isRecepcao, asg: isAsg, encantadora: isEncantadora,
    outros: f => !(isGarcom(f)||isCumim(f)||isBar(f)||isGerente(f)||isMaitre(f)||isRecepcao(f)||isAsg(f)||isEncantadora(f))
  };
...
function _mapaDataBR(di){
  const d = WEEK_DATES[di];
  if(!d) return '';
  return String(d.getDate()).padStart(2,'0')+'/'+String(d.getMonth()+1).padStart(2,'0')+'/'+d.getFullYear();
}

function _mapaFolgaEscala(f, di){
  if(!f) return false;
  if(di === DIAS_IDX[f.folga] || di === DIAS_IDX[f.folga2]) return true;
  if(di === 6){
    const domStr = _mapaDataBR(di);
    const arr = Array.isArray(f.folgaDom) ? f.folgaDom : [];
    if(arr.includes(domStr)) return true;
  }
  return false;
}

function _mapaStatusInfo(status, extra){
  ...
  const cfg = {
    ...
    'saida-antecipada': {txt:'S.ANT', bg:'#f39c1233', color:'#f39c12', conta:true},
    'banco-horas':      {txt:'B.HOR', bg:'#00b89433', color:'#00b894', conta:false},
    'desligado':        {txt:'DESL.', bg:'#e74c3c22', color:'#e74c3c', conta:false},
    'folga':            {txt:'✕',     bg:'',          color:'',        conta:false, folga:true},
    'presente':         {txt:'',      bg:'',          color:'',        conta:true}
  };
  return cfg[st] || null;
}
...
function _mapaCell(f, turno, di){
  ...
  const rec = getRec(di, f.nome);
  const stInfo = rec && rec.status ? _mapaStatusInfo(rec.status, rec.extra||{}) : null;

  // Frequência do dia tem prioridade sobre a escala fixa.
  // Assim, falta/folga/atestado/troca de folga atualizam o mapa na hora.
  if(stInfo){
    if(stInfo.folga){
      return {html:`<td class="cell-folga">✕</td>`, conta:false};
    }
    if(String(rec.status).toLowerCase() === 'presente'){
      return {html:`<td>${_nomeAbrev(f.nome)}</td>`, conta:true};
    }
    return {html:`<td style="background:${stInfo.bg};color:${stInfo.color};font-weight:900;font-size:7px">${stInfo.txt}</td>`, conta:!!stInfo.conta};
  }

  if(_mapaFolgaEscala(f, di)){
    return {html:`<td class="cell-folga">✕</td>`, conta:false};
  }
...
function _mapaTrabalhaDia(f, di){
  ...
  const stInfo = st ? _mapaStatusInfo(st, rec.extra||{}) : null;
  if(stInfo) return {trabalha:!!stInfo.conta, status:st};
  if(_mapaFolgaEscala(f, di)) return {trabalha:false, status:'folga'};
  return {trabalha:true, status:'escala'};
}

function _setorOperacionalLabel(f){
  const c=_normTxt(f.categoria), g=_normTxt(f.grupoEscala || f.grupo || f.setor);
  if(c.includes('GERENTE') || g.includes('GERENCIA')) return 'GERENCIA';
  if(c.includes('MAITRE') || g.includes('MAITRE')) return 'MAITRE';
  if(c.includes('CHEFE DE FILA')) return 'CHEFE DE FILA';
  if(c==='GARCOM' || g.includes('GARCOM')) return 'GARÇONS';
  if(c==='CUMIM' || g.includes('CUMIM')) return 'CUMINS';
  if(['BARMAN','AUXILIAR DE BAR','COPEIRO'].includes(c) || g.includes('BAR')) return 'BAR';
  if(c.includes('RECEPCIONISTA') || g.includes('RECEP')) return 'RECEPÇÃO';
  if(g.includes('CONTROLE') || c.includes('ACESSO')) return 'CONTROLADOR DE ACESSO';
  if(c.includes('ENCANTADORA') || g.includes('BRINQUEDOTECA')) return 'MONITORAS';
  if(c.includes('ASG') || c.includes('LIMPEZA') || g.includes('GOVERNANCA') || g.includes('ASG')) return 'AUX DE LIMPEZA';
  return '';
}
...
function _nomeZapMapa(nome){
  const raw = String(nome||'').trim().replace(/\s+/g,' ');
  if(!raw) return '';
  const norm = _normTxt(raw);
  const mapa = [
    ['WESLHEYN','Weslheyn'],['WESLEY','Wesley'],['EDMAGNA','Edimagna'],['THIAGO','Thiago'],
    ['RICKSON','Rickson'],['WILLIAM','Willian'],['ALEXANDRE','Alexandre'],
    ['SAMUEL','Samuel'],['ELEN','Elen'],['ELLEN','Ellen'],['FABIELLY','Fabielly'],['ANA LUCIA','Ana Lucia'],
    ['FABRICIA','Fabrícia'],['ANDRE','André'],['JOSE FABRICIO','Fabrício'],['IGOR','Igor'],
    ['FERNANDO','Fernando'],['PAULO','Paulo'],['KLEBER','Kleber'],['MIGUEL','Miguel'],['PATRICIA','Patricia'],['CAIO','Caio'],
    ['JOAO GUILHERME','Guilherme'],['GEORGE','George'],['ADRIAN','Adrian'],['FELIPE','Felipe'],['LUCAS','Lucas'],['ISMAEL','Ismael'],['EDUARDO','Eduardo'],
    ['LETICIA','Letícia'],['ANA FLAVIA','Ana Flávia'],['EDLANE','Edlane'],
    ['GABRIELA PAIXAO','Gabriela'],['ISRAEL','Israel'],['NATHALIA','Nathalia'],
    ['BRUNA DAS GRACAS','Bruna'],['KARINE','Karine'],['JESSICA','Jessica'],['FLAVIA','Flavia'],
    ['DANIEL SOARES','Daniel'],['EVANDRO','Evandro'],['RAQUEL','Raquel'],['FABIOLA','Fabiola'],['JORGE','Jorge'],
    ['MARLON','Marlon'],['RHAI','Rhai'],['DANIELLE','Danielle'],['LARISSA','Larissa'],['NELYNE','Nelyne'],['LEONILSON','Leonilson'],['FRANCISCO','Francisco'],['ADALBERTO','Adalberto'],['DANIEL RIBEIRO','Daniel']
  ];
  for(const [k,v] of mapa){ if(norm.includes(k)) return v; }
  const small = ['de','da','do','dos','das','e'];
  return raw.toLowerCase().split(' ').map((w,i)=> small.includes(w)&&i>0 ? w : w.charAt(0).toUpperCase()+w.slice(1)).join(' ');
}

function _linhaTurnoLista(rotulo, nomes, comContagem){
  const lista = (nomes||[]).filter(Boolean).sort((a,b)=>a.localeCompare(b,'pt-BR'));
  if(comContagem){
    if(!lista.length) return rotulo + ': (0)';
    return rotulo + ': (' + lista.length + ')\n' + lista.join('\n');
  }
  return rotulo + ': ' + lista.join(' e ');
}

function _gerarTextoEquipeDiaMapa(){
  const di = curDay;
  const data = WEEK_DATES[di] || new Date();
  const diaNome = ['Segunda','Terça','Quarta','Quinta','Sexta','Sábado','Domingo'][di] || '';
  const dataCurta = String(data.getDate()).padStart(2,'0')+'/'+String(data.getMonth()+1).padStart(2,'0');
  const setoresFixos = ['GERENCIA','MAITRE','CHEFE DE FILA','GARÇONS','CUMINS','RECEPÇÃO','CONTROLADOR DE ACESSO','MONITORAS','AUX DE LIMPEZA','BAR'];
  const turnos = ['ABERTURA','INTERCALADO','FECHAMENTO'];
  const grupos = {};
  setoresFixos.forEach(s=>{ grupos[s] = {ABERTURA:[], INTERCALADO:[], FECHAMENTO:[]}; });

  (EQUIPE||[]).forEach(f=>{
    const info = _mapaTrabalhaDia(f, di);
    if(!info.trabalha) return;
    const setor = _setorOperacionalLabel(f);
    if(!setor || !grupos[setor]) return;
    const turno = _turnoOperacional(f);
    if(!turnos.includes(turno)) return;
    const nome = _nomeZapMapa(f.nome);
    if(nome && !grupos[setor][turno].includes(nome)) grupos[setor][turno].push(nome);
  });

  const linhas = ['📋 Escala – ' + diaNome + ' (' + dataCurta + ')', ''];
  const setoresComContagem = new Set(['GARÇONS','CUMINS','AUX DE LIMPEZA','BAR']);
  setoresFixos.forEach(setor=>{
    const b = grupos[setor];
    const contar = setoresComContagem.has(setor);
    linhas.push(setor + ':');
    if(setor === 'MAITRE') linhas.push(_linhaTurnoLista('Abertura inter', b.ABERTURA, false));
    else linhas.push(_linhaTurnoLista('Abertura', b.ABERTURA, contar));
    linhas.push('');
    linhas.push(_linhaTurnoLista('Intercalado', b.INTERCALADO, contar));
    linhas.push('');
    linhas.push(_linhaTurnoLista('Fechamento', b.FECHAMENTO, contar));
    linhas.push('');
  });
  return linhas.join('\n').replace(/\n{4,}/g,'\n\n\n').trim();
}

function copyMapaEquipeDia(){
  const texto = _gerarTextoEquipeDiaMapa();
  const ok = () => showToast('📋 Equipe do dia copiada para WhatsApp');
  if(navigator.clipboard && window.isSecureContext){
    navigator.clipboard.writeText(texto).then(ok).catch(()=>_copyFallbackMapa(texto, ok));
  } else {
    _copyFallbackMapa(texto, ok);
  }
}

function _copyFallbackMapa(texto, cb){
...
function _mapaCountsFor(grupo){
  const counts = [0,0,0,0,0,0,0];
  (grupo||[]).forEach(f=>{
    const turno = String(f.turno || 'INTERCALADO').toUpperCase();
    DIAS_SEMANA.forEach((_,di)=>{
      if(_mapaCell(f, turno, di).conta) counts[di]++;
    });
  });
  return counts;
}
...
function _renderCountRow(label, counts, rowClass){
  let html = `<tr class="${rowClass||'row-total'}">`;
  counts.forEach((c,i)=>{
    if(i===0) html += `<td><span style="display:block;color:#7a85a3;font-size:7px;font-weight:700;line-height:1">${label}</span><span>${c}</span></td>`;
    else html += `<td>${c}</td>`;
  });
  html += '</tr>';
  return html;
...
function renderMapaEscala(funcs){
  ...
  ordemTurno.forEach(turno => {
    ...
    gr.forEach(f => {
      ...
      html += `<tr class="${cls}">`;
      DIAS_SEMANA.forEach((_,di) => {
        html += _mapaCell(f, turno, di).html;
      });
      html += '</tr>';
    });

    // Não exibe mais contagem isolada da abertura.
    // INTE. = ABERTURA + INTERCALADO
    // FECH. = INTERCALADO + FECHAMENTO
    if(turno === 'INTERCALADO'){
      const ab = _mapaCountsFor(grupos['ABERTURA']);
      const inter = _mapaCountsFor(grupos['INTERCALADO']);
      html += _renderCountRow('INTE.', _sumCounts(ab, inter), 'row-total');
    }

    if(turno === 'FECHAMENTO'){
      const inter = _mapaCountsFor(grupos['INTERCALADO']);
      const fech = _mapaCountsFor(grupos['FECHAMENTO']);
      html += _renderCountRow('FECH.', _sumCounts(inter, fech), 'row-total');
    }
  });

  const abertura = _mapaCountsFor(grupos['ABERTURA']);
  const intercalado = _mapaCountsFor(grupos['INTERCALADO']);
  const fechamento = _mapaCountsFor(grupos['FECHAMENTO']);
  const administrativo = _mapaCountsFor(grupos['ADMINISTRATIVO']);
  const totalPorDia = _sumCounts(abertura, intercalado, fechamento, administrativo);

  html += '<tr class="row-sep"><td colspan="7"></td></tr>';
  html += _renderCountRow('TOTAL', totalPorDia, 'row-grand');
  html += '</tbody></table></div>';
  html += `<div style="display:flex;justify-content:flex-end;gap:8px;padding:8px 12px;border-top:1px solid var(--border);flex-wrap:wrap">
    <button class="mapa-edit-btn" onclick="copyMapaEquipeDia()">📋 Copiar equipe do dia</button>
...
function _mapaParseDataBR(valor){
  if(!valor) return null;
  if(valor instanceof Date && !isNaN(valor)) return new Date(valor.getFullYear(), valor.getMonth(), valor.getDate());
  const txt = String(valor||'').trim();
  let m = txt.match(/^(\d{2})\/(\d{2})\/(\d{4})$/);
  if(m) return new Date(Number(m[3]), Number(m[2])-1, Number(m[1]));
  m = txt.match(/^(\d{2})\/(\d{2})$/);
  if(m){
    const anoBase = (WEEK_DATES && WEEK_DATES[0]) ? WEEK_DATES[0].getFullYear() : new Date().getFullYear();
    return new Date(anoBase, Number(m[2])-1, Number(m[1]));
  }
  return null;
}

function _mapaFmtDataBR(dt){
  if(!dt) return '';
  return String(dt.getDate()).padStart(2,'0')+'/'+String(dt.getMonth()+1).padStart(2,'0')+'/'+dt.getFullYear();
}

function _mapaFmtDataCurta(dt){
  if(!dt) return '';
  return String(dt.getDate()).padStart(2,'0')+'/'+String(dt.getMonth()+1).padStart(2,'0');
}

function _mapaDomingosFixosEscala(){
  // Os domingos do mapa vêm da escala importada, não dos próximos domingos do calendário.
  // Assim 26/04, 03/05, 10/05 etc. permanecem fixos até uma nova escala ser importada.
  const datas = [];
  (EQUIPE||[]).forEach(f=>{
    const arr = Array.isArray(f.folgaDom) ? f.folgaDom : [];
    arr.forEach(v=>{
      const dt = _mapaParseDataBR(v);
      if(dt) datas.push(dt);
    });
  });

  const mapa = new Map();
  datas.forEach(dt=>mapa.set(_mapaFmtDataBR(dt), dt));
  return Array.from(mapa.values()).sort((a,b)=>a-b);
}

function renderMapaDomingo(funcs){
  const domingos = _mapaDomingosFixosEscala();

  if(!domingos.length){
    return '<div class="empty">Nenhuma folga de domingo encontrada na escala importada.</div>';
  }

  // Quem não tem folga dominical fixa e não está em licença.
  const candidatos = funcs.filter(f=>{
    const turno = String(f.turno||'').toUpperCase();
    return f.folga!=='DOM' && f.folga2!=='DOM' && turno!=='LICENÇA' && turno!=='LICENCA';
  });
  ...
  domingos.forEach(dt => html += `<th>${_mapaFmtDataCurta(dt)}</th>`);
  html += '</tr></thead><tbody>';

  // Colunas fixas por domingo da escala. Apenas nomes de quem folga naquele domingo.
  const colunas = domingos.map(dt => {
    const dataStr = _mapaFmtDataBR(dt);
    return candidatos
      .filter(f => _funcionarioFolgaDomingoNaData(f, dt))
      .map(f => _nomeAbrev(f.nome));
  });

  const maxRows = Math.max(...colunas.map(c=>c.length), 1);
...
function openFreelance(modo){
  closeModal('maisMenu');
  const somenteFrequencia = (modo === 'frequencia' || modo === 'frequencia-only' || modo === true);
  const panel = document.getElementById('freelancePanel');
  if(panel) panel.classList.toggle('fl-only-frequencia', somenteFrequencia);
  if(somenteFrequencia) _flSub = 'frequencia';
  else _flSub = 'lista';
...
function _setorNome(s){
  return {garcom:'Garçons',cumim:'Cumins',bar:'Bar',gerente:'Gerente',maitre:'Maitré',recepcao:'Recepção',asg:'Aux. ASG',encantadora:'Encantadora',outros:'Outros'}[s]||s;
}



async function atualizarEquipeOficialAgora(){
  if(!(fbDb && fbConnected)){ showToast('⚠️ Firebase ainda não conectado'); return; }
  const snap = await fbDb.ref(EQUIPE_OFICIAL_PATH).once('value');
  const payload = snap.val();
  if(!payload){ showToast('⚠️ Nenhuma equipe oficial publicada ainda'); return; }
  aplicarEquipeCentral(payload, {silent:false});
  showToast('✅ Equipe oficial atualizada neste dispositivo');
}


// V42 patch aplicado também no script.js separado.
