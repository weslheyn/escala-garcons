...
async function ensureFreelancePlanilha(){
  ...
  if(SHEET_ID_FL){
    try{
      const info=await gFetch(`https://sheets.googleapis.com/v4/spreadsheets/${SHEET_ID_FL}?fields=spreadsheetId,sheets.properties`);
      if(info&&info.spreadsheetId){
  ...
  try{
    ...
    if(search?.files?.length>0){
      ...
      const info2=await gFetch(`https://sheets.googleapis.com/v4/spreadsheets/${SHEET_ID_FL}?fields=sheets.properties`);
      const existentes = (info2?.sheets||[]).map(s=>s.properties.title);
      const faltantes = abas.filter(a=>!existentes.includes(a.title)).map(a=>({addSheet:{properties:{title:a.title,gridProperties:{rowCount:a.rows,columnCount:a.cols}}}}));
      if(faltantes.length){
        await gFetch(`https://sheets.googleapis.com/v4/spreadsheets/${SHEET_ID_FL}:batchUpdate`,'POST',{requests:faltantes});
      }
