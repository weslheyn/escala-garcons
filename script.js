...
function _setorOperacionalLabel(f){
  ...
  if(c.includes('GERENTE') || g.includes('GERENCIA')) return 'GERENTE';
  if(c.includes('MAITRE') || g.includes('MAITRE')) return 'MAITRE';
  if(c==='GARCOM' || c==='CHEFE DE FILA' || g.includes('GARCOM')) return 'GARÇONS';
  ...
  if(c.includes('ASG') || c.includes('LIMPEZA') || g.includes('GOVERNANCA') || g.includes('ASG')) return 'AUX. ASG';
  if(c.includes('ENCANTADORA') || g.includes('BRINQUEDOTECA')) return 'ENCANTADORA';
  if(c.includes('CAIXA') || g.includes('CAIXA')) return 'CAIXA';
  if(c.includes('DELIVERY') || g.includes('DELIVERY')) return 'DELIVERY';
  if(g.includes('ALMOXARIFADO') || c.includes('ALMOXARIFADO')) return 'ALMOXARIFADO';
  if(g.includes('MANUTENCAO') || c.includes('MANUTENCAO')) return 'MANUTENÇÃO';
  if(g.includes('NUTRICAO') || c.includes('NUTRICAO')) return 'NUTRIÇÃO';
  if(g.includes('EVENTOS') || c.includes('EVENTOS')) return 'EVENTOS';
  if(g.includes('CONTROLE') || c.includes('ACESSO')) return 'CONTROLE DE ACESSO';
  if(g.includes('ADMINISTRATIVO') || c.includes('ADMINISTRATIVO')) return 'ADMINISTRATIVO';
  return f.grupoEscala || f.categoria || 'OUTROS';
}
...
function _nomeCompletoMapa(nome){
...
function _gerarTextoEquipeDiaMapa(){
  ...
  const blocos = {ABERTURA:{}, INTERCALADO:{}, FECHAMENTO:{}, ADMINISTRATIVO:{}};
  let total = 0;
  (EQUIPE||[]).forEach(f=>{
    const info = _mapaTrabalhaDia(f, di);
    if(!info.trabalha) return;
    const turno = _turnoOperacional(f);
    if(turno === 'LICENÇA') return;
    const chaveTurno = blocos[turno] ? turno : 'INTERCALADO';
    const setor = _setorOperacionalLabel(f);
    if(!blocos[chaveTurno][setor]) blocos[chaveTurno][setor] = [];
    blocos[chaveTurno][setor].push(_nomeCompletoMapa(f.nome));
    total++;
  });
  const titulos = {ABERTURA:'🟡 ABERTURA', INTERCALADO:'🔵 INTERCALADO', FECHAMENTO:'🔴 FECHAMENTO', ADMINISTRATIVO:'🟢 ADMINISTRATIVO'};
  const ordemSetores = ['GERENTE','MAITRE','GARÇONS','CUMINS','BAR','RECEPÇÃO','AUX. ASG','ENCANTADORA','CAIXA','DELIVERY','ALMOXARIFADO','MANUTENÇÃO','NUTRIÇÃO','EVENTOS','CONTROLE DE ACESSO','ADMINISTRATIVO'];
  const linhas = [`📍 EQUIPE DO DIA — ${diaNome} (${dataCurta})`, ''];
  Object.keys(titulos).forEach(turno=>{
    const setores = blocos[turno];
    const chaves = Object.keys(setores||{});
    if(!chaves.length) return;
    linhas.push(titulos[turno]);
    chaves.sort((a,b)=>{
      const ia=ordemSetores.indexOf(a), ib=ordemSetores.indexOf(b);
      return (ia<0?999:ia)-(ib<0?999:ib) || a.localeCompare(b,'pt-BR');
    }).forEach(setor=>{
      const nomes = setores[setor].sort((a,b)=>a.localeCompare(b,'pt-BR'));
      linhas.push(`• ${setor}: ${nomes.join(', ')}`);
    });
    linhas.push('');
  });
  linhas.push(`👥 TOTAL: ${total} colaboradores`);
  return linhas.join('\n');
}
