...
const STORE='eventos_premium_v1';
const META='eventos_premium_meta_v1';
let deferredInstallPrompt=null;
const STATUS=['Lead','Proposta','Em negociação','Contrato','Fechado','Sem resposta','Cancelado','Perdido'];
...
function statusClass(s){const n=norm(s); if(n.includes('fechado'))return's-fechado'; if(n.includes('perdido'))return's-perdido'; if(n.includes('cancel'))return's-cancelado'; if(n.includes('sem resposta'))return's-semresposta'; if(n.includes('contrato'))return's-contrato'; if(n.includes('proposta'))return's-proposta'; return's-neg';}
...
function calc(list=filtered()){
  ...
  const recuperar=list.filter(e=>['Sem resposta','Cancelado','Perdido'].includes(e.status)).length;
...
function renderRecuperacao(){const list=filtered().filter(e=>['Sem resposta','Cancelado','Perdido'].includes(e.status));$('recuperacao').innerHTML=`<div class="panel"><h3>Clientes para recuperar</h3><p class="muted">Use essa lista para recontato de propostas paradas, canceladas ou sem resposta.</p></div><br><div class="kanban" style="grid-template-columns:repeat(3,minmax(280px,1fr))">${['Sem resposta','Cancelado','Perdido'].map(st=>`<div class="col"><h3>${st} · ${list.filter(e=>e.status===st).length}</h3>${list.filter(e=>e.status===st).map(e=>card(e).replace('</div>','<div class="actions"><button onclick="EVENTOS.whats(\''+e.id+'\')">WhatsApp</button><button onclick="EVENTOS.markRecuperado(\''+e.id+'\')">Recuperado</button></div></div>')).join('')||'<p class="muted">Sem clientes</p>'}</div>`).join('')}</div>`;}
