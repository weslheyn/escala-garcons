// Calendário de Eventos — módulo premium isolado v34
// Firebase otimizado por ano: calendario_eventos/{ano}/eventos
// Mantém o core do app intacto: só lê/escreve no nó calendario_eventos.
let _eventosCache = {};
let _eventosListenerStarted = false; // legado
let _eventosListenersPorAno = {};
let _eventosCachePorAno = {};
let _eventosAnoAtivo = null;
let _eventosCarregandoAno = {};
...
function eventosAnoAtual(){
  const d = _eventosCurrentMonth || eventosParseDate(_eventosSelectedISO) || new Date();
  return d.getFullYear();
}
function eventosArray(){
  const ano = eventosAnoAtual();
  const base = _eventosCachePorAno[ano] || _eventosCache || {};
  return Object.entries(base||{}).map(([id,v])=>eventosNormalizaItem(v,id)).filter(e=>eventosISO(e.data));
}
function eventosDbRoot(){
  if(typeof fbDb!=='undefined' && fbDb) return fbDb.ref('calendario_eventos');
  try{ if(window.firebase && firebase.database) return firebase.database().ref('calendario_eventos'); }catch(e){}
  return null;
}
function eventosDbRef(ano){
  const root = eventosDbRoot();
  if(!root) return null;
  return root.child(String(ano || eventosAnoAtual())).child('eventos');
}
function eventosDbLegacyRef(){
  const root = eventosDbRoot();
  return root ? root.child('eventos') : null;
}
function openCalendarioEventos(){
  ...
  openPanel('eventosPanel'); eventosStartFirebase(false, eventosAnoAtual()); eventosRender();
}
function eventosStartFirebase(force, ano){
  ano = Number(ano || eventosAnoAtual());
  _eventosAnoAtivo = ano;
  const ref=eventosDbRef(ano);
  if(!ref){
    eventosRender();
    try{ showToast('⚠️ Firebase ainda não conectado'); }catch(e){}
    return;
  }
  if(_eventosListenersPorAno[ano] && !force) return;
  try{
    _eventosCarregandoAno[ano]=true;
    if(force && _eventosListenersPorAno[ano]){ try{ ref.off(); }catch(_){} _eventosListenersPorAno[ano]=false; }
    ref.on('value',snap=>{
      const val=snap.val()||{};
      _eventosCachePorAno[ano]=val;
      if(ano===eventosAnoAtual()) _eventosCache=val;
      _eventosCarregandoAno[ano]=false;
      if(Object.keys(val).length===0){ eventosCarregarLegacyPorAno(ano); }
      eventosRender();
      eventosUpdateBadge();
    });
    _eventosListenersPorAno[ano]=true;
    if(force){
      ref.once('value').then(snap=>{
        ...
        eventosRender();
        eventosUpdateBadge();
        const total=Object.keys(_eventosCachePorAno[ano]||{}).length;
        try{ showToast('🔄 '+ano+' atualizado: '+total+' eventos'); }catch(e){}
      });
    }
  }catch(e){
    _eventosCarregandoAno[ano]=false;
...
function eventosCarregarLegacyPorAno(ano){
  const ref=eventosDbLegacyRef(); if(!ref || _eventosCarregandoAno['legacy_'+ano]) return;
  _eventosCarregandoAno['legacy_'+ano]=true;
  ref.once('value').then(snap=>{
    const all=snap.val()||{}, filtrado={};
    Object.entries(all).forEach(([id,v])=>{ const ev=eventosNormalizaItem(v,id); const d=eventosParseDate(ev.data); if(d && d.getFullYear()===Number(ano)) filtrado[id]=v; });
    if(Object.keys(_eventosCachePorAno[ano]||{}).length===0){
      _eventosCachePorAno[ano]=filtrado;
      if(ano===eventosAnoAtual()) _eventosCache=filtrado;
      eventosRender(); eventosUpdateBadge();
    }
  }).finally(()=>{ _eventosCarregandoAno['legacy_'+ano]=false; });
}
function eventosAtualizarManual(){
  eventosStartFirebase(true, eventosAnoAtual());
}
...
function eventosSetDate(iso){ _eventosSelectedISO=iso; const d=eventosParseDate(iso); if(d){ _eventosCurrentMonth=new Date(d.getFullYear(),d.getMonth(),1); eventosStartFirebase(false,d.getFullYear()); } const inp=document.getElementById('evDatePicker'); if(inp) inp.value=iso; eventosRender(); }
function eventosMoveMonth(delta){ const d=_eventosCurrentMonth||new Date(); _eventosCurrentMonth=new Date(d.getFullYear(),d.getMonth()+delta,1); _eventosSelectedISO=eventosDateToISO(new Date(_eventosCurrentMonth.getFullYear(),_eventosCurrentMonth.getMonth(),1)); eventosStartFirebase(false,_eventosCurrentMonth.getFullYear()); const inp=document.getElementById('evDatePicker'); if(inp) inp.value=_eventosSelectedISO; eventosRender(); }
...
function eventosRender(){
  ...
  p.innerHTML=`<div class="ev-premium-wrap">
    <div class="ev-hero"><div class="ev-hero-title">Agenda comercial e operacional</div><div class="ev-hero-main"><div><b>Calendário de Eventos</b><span id="evSubMonth">${eventosMesNome(_eventosCurrentMonth)}</span></div><span id="evSyncInfo">${_eventosCarregandoAno[eventosAnoAtual()] ? "Carregando " + eventosAnoAtual() + "..." : "Tempo real"}</span></div></div>
...
function eventosSalvarModal(){
  ...
  const ref=eventosDbRef(dt.getFullYear()); if(!ref){ showToast('Firebase indisponível'); return; } ref.child(id).set(item).then(()=>{showToast('✅ Evento salvo'); eventosCloseModal();}).catch(e=>alert('Erro ao salvar evento: '+e.message));
}
function eventosExcluirAtual(){ const id=document.getElementById('evFormId').value; if(!id||!confirm('Excluir este evento?')) return; const data=document.getElementById('evFormData').value||eventosHojeISO(); const dt=eventosParseDate(data)||new Date(); const ref=eventosDbRef(dt.getFullYear()); if(!ref) return; ref.child(id).remove().then(()=>{showToast('🗑️ Evento excluído'); eventosCloseModal();}); }
function eventosMostrarAjuda(){ alert('Integração Google Planilhas → Firebase:\n\n1. Abra a planilha de eventos.\n2. Extensões > Apps Script.\n3. Cole o arquivo AppsScript_Calendario_Eventos_Isolado.gs deste ZIP.\n4. Configure a URL do Firebase, se necessário.\n5. Execute sincronizarCalendarioEventosVisual().\n6. Crie gatilho instalável onEdit ou sincronização a cada 1 minuto.\n\nO app lê calendario_eventos/{ano}/eventos em tempo real. Essa versão é otimizada por ano para carregar 2026 e 2027 sem atraso.'); }
