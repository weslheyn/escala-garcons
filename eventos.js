...
function renderFunil(){const list=filtered().filter(e=>!isRecuperacaoStatus(e.status));$('funil').innerHTML=`<div class="pipeline-note panel"><h3>Funil de negociação</h3><p class="muted">Lead → Proposta enviada → Visita do espaço → Negociação 1 → Negociação 2 → Reunião de alinhamento → Contrato enviado → Assinatura cliente → Assinatura diretoria → Fechado → Realizado.</p></div><br><div class="kanban pipeline-kanban">${PIPELINE_STATUS.map(st=>`<div class="col"><h3>${st} · ${list.filter(e=>e.status===st).length}</h3>${list.filter(e=>e.status===st).map(card).join('')||'<p class="muted">Sem eventos</p>'}</div>`).join('')}</div>`;}
