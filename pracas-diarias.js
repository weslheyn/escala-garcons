/* =====================================================
   MÓDULO PRAÇAS DIÁRIAS E SORTEIO — v37
...
(function(){
  ...
  function renderConfig(){
    ...
    const areas=[...new Set(PRACAS_PADRAO.map(p=>p.area))];
  ...
  window.toggleFechamentoAlmocoPracas = function(){ st.incluirFechamentoNoAlmoco=!st.incluirFechamentoNoAlmoco; renderAll(); toast(st.incluirFechamentoNoAlmoco?'✅ Fechamento incluído no almoço':'F/ removido do almoço'); };
  window.toggleSalaoPracas = function(encodedArea){ const area=decodeURIComponent(encodedArea); st.saloesAtivos[area]=!isSalaoAtivo(area); renderAll(); toast((st.saloesAtivos[area]?'✅ ':'🚫 ')+area); };
