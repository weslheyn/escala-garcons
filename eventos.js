...
let state={tab:'dashboard',eventos:[],agenda:[],agendaFilter:{visibilidade:'todos',tipo:'todos',status:'todos'},pacotes:window.EVENTOS_PACOTES||[],meta:{metaMensal:150000},cal:{year:new Date().getFullYear(),month:new Date().getMonth()+1}};
...
function currentUser(){return localStorage.getItem('eventos_usuario_nome')||'';}
...
function agendaPessoas(){return [...new Set([...agendaVisibleList().map(a=>a.criador).filter(Boolean), currentUser()].filter(Boolean))];}
function filtered(){
...
function agendaFiltered(){
  ...
  return agendaVisibleList().filter(a=>{
    if(f.visibilidade==='pessoal' && a.visibilidade==='compartilhada') return false;
    if(f.visibilidade==='compartilhado' && a.visibilidade!=='compartilhada') return false;
    if(f.tipo&&f.tipo!=='todos'&&a.tipo!==f.tipo)return false;
    if(f.status&&f.status!=='todos'&&agendaStatus(a).toLowerCase()!==f.status)return false;
    return true;
  });
}
  ...
  $('agenda').innerHTML=`
    <div class="agenda-page">
      ...
      <div class="agenda-filters premium-block">
        <button class="chip ${state.agendaFilter.visibilidade==='todos'?'active':''}" onclick="EVENTOS.agendaFilter('visibilidade','todos')">Todos</button>
        <button class="chip ${state.agendaFilter.visibilidade==='pessoal'?'active':''}" onclick="EVENTOS.agendaFilter('visibilidade','pessoal')">Pessoal</button>
        <button class="chip ${state.agendaFilter.visibilidade==='compartilhado'?'active':''}" onclick="EVENTOS.agendaFilter('visibilidade','compartilhado')">Compartilhado</button>
        ...
        <select class="field agenda-select" onchange="EVENTOS.agendaFilter('status',this.value)"><option value="todos">Todos os status</option><option value="pendente" ${state.agendaFilter.status==='pendente'?'selected':''}>Pendentes</option><option value="atrasada" ${state.agendaFilter.status==='atrasada'?'selected':''}>Atrasadas</option><option value="concluída" ${state.agendaFilter.status==='concluída'?'selected':''}>Concluídas</option></select>
      </div>
