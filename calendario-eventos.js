// Calendário de Eventos — módulo premium isolado v32
...
function eventosDbRef(){ return (typeof fbDb!=='undefined' && fbDb) ? fbDb.ref('calendario_eventos/eventos') : null; }
...
function eventosStartFirebase(force){
  if(_eventosListenerStarted && !force) return; const ref=eventosDbRef(); if(!ref){ eventosRender(); return; }
  _eventosListenerStarted=true;
  try{ ref.off(); ref.on('value',snap=>{_eventosCache=snap.val()||{}; eventosRender(); eventosUpdateBadge();}); }catch(e){console.warn('eventosStartFirebase',e)}
}
