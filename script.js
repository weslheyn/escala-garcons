...
async function ensureFreelancePlanilha(){
  ...
  try{
    ...
    const search=await gFetch(`https://www.googleapis.com/drive/v3/files?q=${q}&fields=files(id,name,webViewLink)`);
    if(search?.files?.length>0){
...
function triggerFreelanceSheetsSync(){
  if(gToken&&SHEET_ID_FL){
    ...
    window._freelanceSheetsTimer=setTimeout(()=>syncFreelanceSheets().catch(()=>{}),2500);
  }
}
...
function _flTotalCampos(flId){
  ...
  const raw = Object.values(_flSlotsCache[flId] || {});
  const preenchidos = raw.filter(s=>s && s.status).length;
  const presentes = raw.filter(s=>s && s.status==='presente').length;
  // Sempre deixar campos suficientes para completar 8 presenças.
  // Ex.: se houver 1 falta, abre 1 campo extra automaticamente.
  return Math.max(meta, Math.min(31, preenchidos + Math.max(0, meta - presentes)));
}
...
function _flExpandSlotsIfNeeded(flId){
  ...
  if(fbDb&&fbConnected){
    for(let i=1;i<=needed;i++){
      if(!_flSlotsCache[flId][i]) _flSlotsCache[flId][i] = {ordem:i, data:'', status:'', ts:0};
      fbDb.ref('freelances_slots/'+flId+'/'+i).update(_flSlotsCache[flId][i]).catch(()=>{});
    }
  }
}
...
async function _carregarFreelancesFirebase(){
  ...
  try{
    ...
    Object.values(_flCache).forEach(fl=>{
      ...
      _flEnsureSlots(fl.id, totalPadrao);
...
function salvarFreelance(){
  ...
  const fl = {id, nome, funcao, dataIni, dias, totalDiarias:dias, metaPresencas:dias, valor, auth, fechamentoPrevisto:7, fechamentoModo:(_flCache[idExist]?.fechamentoModo||''), criadoEm:idExist?(_flCache[idExist]?.criadoEm||Date.now()):Date.now()};
  _flCache[id]=fl;
