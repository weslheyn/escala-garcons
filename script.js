...
function buildTurno(t){
  const cards=t.membros.map(f=>{
    ...
    if(isFolgaSemanal(f,DAYS[curDay])){
      ...
      // Só mostra card normal se o dia atual realmente for o dia trabalhado.
      if(folgaRec){
        const todayIso=WEEK_DATES[curDay].toISOString().split('T')[0];
        const workedIso=folgaRec.trabalhouIso||todayIso;
        if(folgaRec.tipo==='presente' || folgaRec.tipo==='compra' || (folgaRec.tipo==='troca' && workedIso===todayIso)){
...
function buildFolgaCard(f, dayIdx){
  ...
  if(folgaRec){
    if(folgaRec.tipo==='presente'){
      ...
      if(folgaRec.trabalhouFmt||folgaRec.trabalhouIso)extraInfo+=`<div style="font-size:10px;color:#1abc9c;margin-top:2px">💼 Trabalha: ${folgaRec.trabalhouFmt||_isoToPtBr(folgaRec.trabalhouIso)}</div>`;
      if(folgaRec.trocaDiaFmt||folgaRec.folgaIso)extraInfo+=`<div style="font-size:10px;color:var(--sub);margin-top:2px">📅 Folga: ${folgaRec.trocaDiaFmt||_isoToPtBr(folgaRec.folgaIso)}${folgaRec.trocaDiaSem?' ('+folgaRec.trocaDiaSem+')':''}</div>`;
...
function saveDomFolga(){
  ...
  if(_domTipo==='presente'){
    ...
    if(oldRec&&oldRec.tipo==='troca'){
      const oldWork=oldRec.trabalhouIso||workedIso;
      const oldFolga=oldRec.trocaDia||oldRec.folgaIso||'';
      if(oldWork!==trabDia || oldFolga!==trocaDia){
        _removeTrocaLinks(nome, oldWork, oldFolga);
      }
    }
