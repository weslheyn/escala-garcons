...
(function(){
  ...
  function filteredPracas(){
    let arr=getPracas().filter(p=>!st.pracasBloqueadas[p.id]);
  ...
  function fazerSorteio(){
    ...
    const prs=getPracas().filter(p=>!st.pracasBloqueadas[p.id]);
  ...
  function payloadResultado(){
    ...
    return {id:r.id||('pr_'+Date.now()), data:st.data, turno:st.turno, atualizadoEm:new Date().toISOString(), origem:'app_mapa',
      normal:conv(r.normal), cf:conv(r.cf), fechamento:conv(r.fechamento), sobraram:r.sobraram||{}, travas:st.pracaLocks, pracasBloqueadas:st.pracasBloqueadas, tipoSorteio:st.tipoSorteio, resumo:r.resumo||{} };
  }
  ...
  function renderConfig(){
    ...
    el.innerHTML=`
      ...
      <div class="pracas-section-title"><span>Praças fora do sorteio</span><span class="pracas-count">${totalBloq} bloqueadas</span></div>
      <div class="pracas-list">${getPracas().map(p=>`<div class="pracas-row"><div class="pracas-num">${p.numero}</div><div><div class="pracas-name">Praça ${p.numero}</div><div class="pracas-sub">${p.area} • Mesas: ${p.mesas}</div></div><button class="pracas-pill ${st.pracasBloqueadas[p.id]?'lock':''}" onclick="toggleBloqueioPraca('${p.id}')">${st.pracasBloqueadas[p.id]?'🚫 Bloqueada':'Disponível'}</button></div>`).join('')}</div>
      <div class="pracas-actions"><button class="pracas-btn red" onclick="limparBloqueiosPracas()">🚫 Limpar bloqueios</button><button class="pracas-btn" onclick="setPracasView('resultado')">📋 Ver escala</button></div>
    `;
  }
  ...
  function pracaElegivelIndividual(grupo){
    let prs=getPracas().filter(p=>!st.pracasBloqueadas[p.id] && !isPracaAtribuida(p.id) && tipoParaGrupo(p.tipo)===grupo);
    if(st.turno==='almoco') prs=prs.filter(p=>p.tipo!=='F');
    return prs;
  }
  ...
  function renderSorteioIndividual(){
    ...
    return `<div class="pracas-section-title"><span>Sorteio individual</span><span class="pracas-count">cada pessoa sorteia a sua praça</span></div>
      ...
      ${grupos.map(([g,label])=>{
        if(st.turno==='almoco' && g==='fechamento') return '';
  ...
  function areaRows(){
    ...
    getPracas().forEach(p=>{ if(!cur || cur.area!==p.area){ cur={area:p.area,rows:[]}; groups.push(cur); } cur.rows.push(p); });
    return groups;
  }
  function renderEscalaPracas(showActions){
    ...
    </div>${showActions?`<div class="pracas-print-actions no-print"><button class="pracas-btn" onclick="imprimirEscalaPracas()">🖨️ Imprimir / PDF</button><button class="pracas-btn" onclick="copiarEscalaPracasTabela()">📋 Copiar escala</button></div>`:''}`;
  }
  ...
  window.setTipoSorteioPracas = function(tipo){ st.tipoSorteio=tipo||'geral'; renderAll(); toast(tipo==='individual'?'👆 Sorteio individual ativado':'🎲 Sorteio geral ativado'); };
  window.toggleBloqueioPraca = function(pracaId){ if(st.pracasBloqueadas[pracaId]) delete st.pracasBloqueadas[pracaId]; else st.pracasBloqueadas[pracaId]=true; renderPracas(); renderConfig(); renderResultado(); };
  window.limparBloqueiosPracas = function(){ if(confirm('Liberar todas as praças bloqueadas?')){st.pracasBloqueadas={}; renderAll(); toast('✅ Praças liberadas');} };
  ...
  window.imprimirEscalaPracas = function(){ window.print(); };
