...
function openFolgaModalById(f, dayIdx, editMode, folgaKey){
  ...
  ['dom_pres_obs','dom_comp_valor','dom_comp_auth','dom_comp_obs',
   'dom_troca_trabalha','dom_troca_dia','dom_troca_auth','dom_troca_obs','dom_falta_obs',
  ...
  document.getElementById('dom_troca_trabalha').value=folgaDate.toISOString().split('T')[0];
  ...
  if(folgaRec&&editMode){
    ...
    if(folgaRec.tipo==='compra'){
      ...
      document.getElementById('dom_troca_trabalha').value=folgaRec.trabalhaDia||workedIso;
...
function saveDomFolga(){
  ...
  if(_domTipo==='presente'){
    ...
    const trabalhaDia=v('dom_troca_trabalha') || workedIso;
    const trocaDia=v('dom_troca_dia');
    if(!trabalhaDia){showToast('⚠️ Preencha o dia que vai trabalhar');return;}
    if(!trocaDia){showToast('⚠️ Selecione a data da folga');return;}
    if(trabalhaDia===trocaDia){showToast('⚠️ O dia de trabalho e o dia da folga não podem ser iguais');return;}
    const fmt2=d=>{const p=d.split('-');return p[2]+'/'+p[1]+'/'+p[0];};
    rec.trabalhaDia=trabalhaDia;
    rec.trabalhaDiaFmt=fmt2(trabalhaDia);
    ...
    if(oldRec&&oldRec.tipo==='troca'&&oldRec.trocaDia&&oldRec.trocaDia!==trocaDia){
      _removeTrocaLinks(nome, oldRec.trabalhaDia||workedIso, oldRec.trocaDia);
    }

    // 1) DIA DE TRABALHO: marca PRESENTE com obs TROCA FOLGA
    const obsHoje='TROCA FOLGA — trabalhou em '+rec.trabalhaDiaFmt+' e folga em '+rec.trocaDiaFmt+(rec.obs?' | '+rec.obs:'');
    if(trabalhaDia===workedIso){
      setRec(dayIdx,nome,'presente',{domTipo:'troca',trabalhaEm:rec.trabalhaDiaFmt,folgaEm:rec.trocaDiaFmt,obs:obsHoje,folgaTrabalhada:true});
      bridgeSaveByDate(nome,dayIdx,'presente',{domTipo:'troca',trabalhaEm:rec.trabalhaDiaFmt,folgaEm:rec.trocaDiaFmt,obs:obsHoje,folgaTrabalhada:true});
    } else {
      const workKey='drec_'+nome+'_'+trabalhaDia;
      const workVal={status:'presente',extra:{domTipo:'troca',trabalhaEm:rec.trabalhaDiaFmt,folgaEm:rec.trocaDiaFmt,obs:obsHoje,folgaTrabalhada:true},ts:Date.now()};
      save(workKey,workVal);
      pushToFirebase(workKey,workVal);
    }

    // 2) DATA ESCOLHIDA: marca como TROCA-FOLGA
    const obsFolga='TROCA FOLGA — trabalhou em '+rec.trabalhaDiaFmt;
    const drKey='drec_'+nome+'_'+trocaDia;
    const drVal={status:'troca-folga',extra:{obs:obsFolga,trocaDe:rec.trabalhaDiaFmt,trabData:trabalhaDia,trab:rec.trabalhaDiaFmt,folgaData:trocaDia,folga:rec.trocaDiaFmt},ts:Date.now()};
    save(drKey,drVal);
    ...
    const trocaRecord={
      ...
      trabalhouIso:trabalhaDia, trabalhouFmt:rec.trabalhaDiaFmt,
    ...
    showToast('✅ Trabalha em '+rec.trabalhaDiaFmt+' · folga em '+rec.trocaDiaFmt);
