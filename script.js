...
function isFolgaEscala(f,diaSem,dataObj){
  ...
  const dt=dataObj instanceof Date ? dataObj : null;
  if(ds==='DOM'){
    const arr=Array.isArray(f.folgaDom)?f.folgaDom.filter(Boolean):[];
    // Se a escala importada trouxe domingos específicos, eles mandam no domingo.
    // Isso corrige casos como Paulo/Alexandre: folga do domingo vem da tabela DOMINGO DO MÊS.
    if(arr.length && dt){
      const alvo=_fmtDataBRFolga(dt);
      const ano=dt.getFullYear();
      return arr.some(v=>_normDataFolgaValor(v,ano)===alvo);
    }
    // Administrativo ou escala explícita DOM continua respeitando DOM.
    return f.folga==='DOM'||f.folga2==='DOM';
  }
...
function renderMapaDomingo(funcs){
  ...
  const colunas = domingos.map(dt => {
    ...
    return candidatos
      .filter(f => (Array.isArray(f.folgaDom) ? f.folgaDom : []).includes(dataStr))
