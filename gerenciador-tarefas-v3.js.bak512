...
function renderList(l){const el=document.createElement('div'); const isCollapsed=!!l.collapsed; el.className='gt-list'+(isCollapsed?' is-collapsed':''); el.dataset.listId=l.id; const cardIds=(l.cards||[]).filter(id=>state.cards[id]); const cards=cardIds.map(id=>normalizeCardForModal(state.cards[id],id)).filter(Boolean); el.innerHTML=`<div class="gt-list-head"><button class="gt-list-toggle" title="${isCollapsed?'Mostrar lista':'Ocultar lista'}" aria-label="${isCollapsed?'Mostrar lista':'Ocultar lista'}">${isCollapsed?'▸':'▾'}</button><input class="gt-list-title" value="${escAttr(l.title)}" title="${escAttr(l.title)}"><span class="gt-list-count">${cards.length}</span><button class="gt-list-menu">•••</button></div><div class="gt-cards"></div><button class="gt-add-card">+ Adicionar um cartão</button>`; const toggle=el.querySelector('.gt-list-toggle'); toggle.onclick=(e)=>{e.stopPropagation(); l.collapsed=!l.collapsed; save(); renderBoard();}; const title=el.querySelector('.gt-list-title'); title.onchange=()=>{l.title=title.value.trim()||'Lista'; save(); renderBoard()}; const cont=el.querySelector('.gt-cards'); if(!isCollapsed){cards.forEach(c=>cont.appendChild(renderCard(c,l.id))); el.querySelector('.gt-add-card').onclick=()=>openCardForm(l.id); makeDrop(cont,l.id);} else {cont.innerHTML='';} el.querySelector('.gt-list-menu').onclick=()=>openListMenu(l.id); return el;}
