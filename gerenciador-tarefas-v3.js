...
function fireCardAlarm(c,test=false){ const a=effectiveAlarm(c)||{sound:true,vibrate:true,scope:'local',time:''}; if(a.sound!==false) playAlarmSound(a.soundId||'default'); if(a.vibrate!==false && navigator.vibrate) navigator.vibrate([280,120,280,120,280]); const tipo=a.scope==='shared'?'Alarme compartilhado':'Alarme deste dispositivo'; const soundName=getAlarmSound(a.soundId||'default').name; const body=`${tipo} • ${a.time||''} • ${soundName}${test?' • Teste':''}`; if('Notification' in window && Notification.permission==='granted'){try{new Notification('Gerenciador de Tarefas', {body:`${c.title}\n${body}`, icon:'icon.png', badge:'icon.png'});}catch(e){toast('⏰ '+c.title)}} else toast('⏰ '+c.title); }
