...
const visualThemes=[
 {id:'default',name:'Padrão atual',desc:'Claro, simples e leve'},
 {id:'premium',name:'Premium Glass',desc:'Vidro escuro translúcido'},
 {id:'executive',name:'Executivo Dark',desc:'Preto fosco com dourado'},
 {id:'coco',name:'Coco Bambu',desc:'Bordô, marrom e dourado'},
 {id:'corporate',name:'Corporativo',desc:'Azul limpo e moderno'},
 {id:'neon',name:'Neon',desc:'Roxo e azul tecnológico'}
];
...
function normalizeVisualTheme(v){return visualThemes.some(t=>t.id===v)?v:'default';}
function normalizeDensity(v){return densityOptions.some(t=>t.id===v)?v:'normal';}
function applyBoardAppearance(b){
 const app=$('#gtApp'); if(!app||!b)return;
 app.classList.remove('gt-theme-default','gt-theme-premium','gt-theme-executive','gt-theme-coco','gt-theme-corporate','gt-theme-neon','gt-density-compact','gt-density-normal','gt-density-comfortable');
 b.visualTheme=normalizeVisualTheme(b.visualTheme);
 b.density=normalizeDensity(b.density);
 app.classList.add('gt-theme-'+b.visualTheme,'gt-density-'+b.density);
}
...
function firstFileCardForBoard(b){
  if(!b) return null;
  for(const lid of (b.lists||[])){
    const l=state.lists[lid];
    for(const cid of (l?.cards||[])){
      const c=state.cards[cid];
      if(c && c.type==='file' && c.file) return c;
    }
  }
  return null;
}
function applyBoardTileCover(el,b){
  const file=firstFileCardForBoard(b);
  if(file && file.file && file.file.thumb){
    el.style.backgroundImage=`linear-gradient(180deg,rgba(0,0,0,.10),rgba(0,0,0,.68)),url("${file.file.thumb}")`;
    el.style.backgroundSize='cover'; el.style.backgroundPosition='center';
    el.classList.add('has-file-cover');
  }else{
    setBg(el,b.background);
  }
}
...
function getListDropIndex(wrap,x){const lists=[...wrap.querySelectorAll('.gt-list:not(.list-dragging)')]; let closest={offset:Number.NEGATIVE_INFINITY,index:lists.length}; lists.forEach((list,i)=>{const box=list.getBoundingClientRect(); const offset=x-box.left-box.width/2; if(offset<0 && offset>closest.offset){closest={offset,index:i};}}); return closest.index;}
...
async function makeFileThumbBlobSafe(file,kind){
 try{
  ...
  if(kind==='excel'||kind==='word'||kind==='powerpoint'||kind==='file') return await makeOfficeThumbBlob(file,kind);
 }catch(e){console.warn('Não foi possível gerar thumbnail',e);}
 try{return await makeOfficeThumbBlob(file,kind||'file');}catch(e){return null;}
}
...
function makeOfficeThumbBlob(file,kind='file'){
 const canvas=document.createElement('canvas'); canvas.width=520; canvas.height=300;
 const ctx=canvas.getContext('2d',{alpha:false});
 const palette={excel:['#052e16','#16a34a','#86efac','EXCEL'],word:['#0f172a','#2563eb','#93c5fd','WORD'],powerpoint:['#431407','#ea580c','#fdba74','POWER'],pdf:['#450a0a','#ef4444','#fecaca','PDF'],file:['#111827','#64748b','#cbd5e1','ARQUIVO']};
 const p=palette[kind]||palette.file;
 const grd=ctx.createLinearGradient(0,0,520,300); grd.addColorStop(0,p[0]); grd.addColorStop(1,'#020617');
 ctx.fillStyle=grd; ctx.fillRect(0,0,520,300);
 ctx.fillStyle='rgba(255,255,255,.06)'; for(let x=0;x<520;x+=46){ctx.fillRect(x,0,1,300)} for(let y=0;y<300;y+=42){ctx.fillRect(0,y,520,1)}
 ctx.fillStyle=p[1]; ctx.beginPath(); ctx.roundRect(36,36,112,112,20); ctx.fill();
 ctx.fillStyle='rgba(255,255,255,.18)'; ctx.fillRect(64,58,56,7); ctx.fillRect(64,78,56,7); ctx.fillRect(64,98,42,7);
 ctx.fillStyle='#ffffff'; ctx.font='900 42px Arial, sans-serif'; ctx.fillText(p[3],170,92);
 ctx.fillStyle=p[2]; ctx.font='700 20px Arial, sans-serif'; ctx.fillText((file.name||'arquivo').slice(0,34),170,132);
 ctx.fillStyle='rgba(255,255,255,.72)'; ctx.font='600 16px Arial, sans-serif'; ctx.fillText(fmtBytes(file.size||0),170,164);
 return canvasToBlob(canvas,'image/webp',.72);
}
...
async function deleteFileCard(cardId){
 const c=state.cards[cardId]; if(!c||!c.file)return;
 if(!confirm('Excluir este arquivo?')) return;
 const f=c.file||{};
 try{
  const storage=window.GerenciadorTarefasFirebase;
  if(storage&&storage.deleteStorage){
   if(f.storagePath) await storage.deleteStorage(f.storagePath);
   if(f.thumbPath) await storage.deleteStorage(f.thumbPath);
  }
 }catch(e){console.warn('Falha ao excluir do Storage',e);}
 deleteCard(cardId);
...
function openBackground(){
 ...
 b.visualTheme=normalizeVisualTheme(b.visualTheme);
 b.density=normalizeDensity(b.density);
 const m=$('#gtBackgroundModal');
 m.innerHTML=`<button class="gt-close" data-close>×</button>
 <h2>Aparência do quadro</h2>
 <p class="gt-muted">Escolha o tema visual, a imagem de fundo e a densidade da tela. O tema não altera suas listas, cards ou dados do Firebase.</p>
 <h3>Temas visuais</h3>
 <div class="gt-theme-grid">${visualThemes.map(t=>`<button class="gt-theme-option ${b.visualTheme===t.id?'selected':''}" data-theme="${t.id}"><strong>${esc(t.name)}</strong><small>${esc(t.desc)}</small></button>`).join('')}</div>
 <h3>Fundos personalizados</h3>
 <div class="gt-background-grid">${bgLib.map(bg=>`<button class="gt-bg-option ${b.background?.id===bg.id?'selected':''}" data-bg="${bg.id}"><span>${esc(bg.name)}</span></button>`).join('')}</div>
 <div class="gt-field"><label>Imagem personalizada por URL</label><input id="customBg" placeholder="https://..."></div>
 <button class="gt-secondary" id="saveCustomBg">Usar imagem por URL</button>
 <h3>Densidade visual</h3>
 <div class="gt-density-grid">${densityOptions.map(d=>`<button class="gt-density-option ${b.density===d.id?'selected':''}" data-density="${d.id}">${esc(d.name)}</button>`).join('')}</div>
 <div class="gt-form-actions"><button class="gt-primary" data-close>Concluir</button></div>`;
 m.classList.remove('gt-hidden');
 bgLib.forEach(bg=>{const el=m.querySelector(`[data-bg="${bg.id}"]`); setBg(el,bg); if(el)el.onclick=()=>{b.background=bg; save(); renderBoard(); openBackground();};});
 $$('.gt-theme-option').forEach(btn=>btn.onclick=()=>{b.visualTheme=btn.dataset.theme; save(); renderBoard(); openBackground();});
 $$('.gt-density-option').forEach(btn=>btn.onclick=()=>{b.density=btn.dataset.density; save(); renderBoard(); openBackground();});
 $('#saveCustomBg').onclick=()=>{const v=$('#customBg').value.trim(); if(v){b.background={id:'custom',name:'Personalizado',type:'image',value:v}; save(); closeModals(); renderBoard();}};
